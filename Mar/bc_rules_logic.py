...
class ExcludedResult:
    ...
    hostname:         str
    annotation:       str   # annotation value that triggered exclusion
...
def _is_excluded(annotation):
    # Return True if annotation starts with 'excl' (case-insensitive)
    if not annotation:
        return False
    return annotation.strip().upper().startswith('EXCL')
...
def compute_bc_results(servers_qs, datacenter: str, rule_texts: dict):
    """
    ...
    Returns:
        Tuple (results, excluded):
          - results  : list of BCResult, one per processable ServerUnique
          - excluded : list of ExcludedResult for servers skipped due to annotation
    """
    ...
    for server in servers_qs.select_related('server_unique'):
        ...
        # Exclude servers whose annotation starts with 'excl' (case-insensitive)
        if _is_excluded(su.annotation):
