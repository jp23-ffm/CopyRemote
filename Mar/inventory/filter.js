document.addEventListener('DOMContentLoaded', function () {
	...
	if (saveSearchForm) {
		...
		saveSearchForm.addEventListener('submit', function(event) {
        	...
        	const columnOrder = sessionStorage.getItem(COLUMN_ORDER_KEY);
        	if (columnOrder) {
    ...
    for (const [key, value] of urlParams.entries()) {
        ...
        // Skip date range params \97 restored separately below
    ...
    const COLUMN_ORDER_KEY = (window.location.pathname.replace(/\/+$/, '').split('/').filter(Boolean).join('_') || 'page') + '_columnOrder';
    ...
    function saveColumnOrder(table, fixedCols) {
        const order = Array.from(table.querySelectorAll('thead th'))
            ...
            .filter(Boolean);
        sessionStorage.setItem(COLUMN_ORDER_KEY, JSON.stringify(order));
    }
    ...
    function restoreColumnOrder() {
        ...
        if (orderStr) {
            ...
            orderStr = sessionStorage.getItem(COLUMN_ORDER_KEY);
        }
        if (!orderStr) return;
