...
from common.query_profiler import profile_queries


app_name = __package__.split('.')[-1]
logger = logging.getLogger('inventory')

# Keep IN(...) clauses well under SQLite's bound-variable limit (32766 on
# SQLite >= 3.32, which Django 6 / modern Python bundle). Too small a chunk
# turns one query into hundreds of sequential round-trips and becomes the
# actual bottleneck — 5000 stays safely under the limit while keeping the
# round-trip count low even for very large exports.
QUERY_CHUNK_SIZE = 5000

# Caps how many exports can stream their file at once, per gunicorn worker
# process, so concurrent big exports don't stack their memory usage.
...
def batch_load_listbox_values(listbox_fields, su_fields=None, force_refresh=False):
    ...
    for field in listbox_fields:
        ...
        if not force_refresh:
            ...
            cached = listbox_cache.get(cache_key)
    ...
    with connection.cursor() as cursor:
        for field in fields_to_query:
            ...
            listbox_cache.set(f'{cache_key_prefix}{field}', values, 3600)
...
def _apply_char_date_filter(queryset, field_name, lookup, iso_value):
    """Add a WHERE clause comparing a dd.mm.yyyy or yyyy-mm-ddT... CharField against an ISO date string."""
    op = '>=' if lookup == 'gte' else '<='
    if connection.vendor == 'sqlite':
        sql = (
            f'(CASE '
            f'WHEN "{field_name}" GLOB \'??.??.????\' THEN '
            f'SUBSTR("{field_name}", 7, 4) || \'-\' || SUBSTR("{field_name}", 4, 2) || \'-\' || SUBSTR("{field_name}", 1, 2) '
            f'WHEN "{field_name}" GLOB \'????-??-??T*\' THEN SUBSTR("{field_name}", 1, 10) '
            f'ELSE NULL END) {op} %s'
        )
    else:
        sql = (
            f'(CASE '
            f'WHEN "{field_name}" ~ \'^\\d{{2}}\\.\\d{{2}}\\.\\d{{4}}$\' THEN '
            f'SUBSTR("{field_name}", 7, 4) || \'-\' || SUBSTR("{field_name}", 4, 2) || \'-\' || SUBSTR("{field_name}", 1, 2) '
            f'WHEN "{field_name}" ~ \'^\\d{{4}}-\\d{{2}}-\\d{{2}}T\' THEN SUBSTR("{field_name}", 1, 10) '
            f'ELSE NULL END) {op} %s'
        )
    return queryset.extra(where=[sql], params=[iso_value])
...
def apply_filters_to_queryset(queryset, filters, su_fields=None):
    ...
    for key, values in filters.items():
        ...
        if key.endswith('__gte') or key.endswith('__lte'):
            ...
            try:
                d = datetime.datetime.strptime(values[0], '%Y-%m-%d').date()
            except (ValueError, TypeError):
                # Non-date value (e.g. "0.0"): exact string match on the raw field
                combined_filter_query &= Q(**{field_name: values[0]})
                continue
            queryset = _apply_char_date_filter(queryset, field_name, lookup, d.isoformat())
        else:
...
def server_view(request):
    ...
    profiler.enable()
    response = _server_view_impl(request)
    profiler.disable()

    s = io.StringIO()
    ps = pstats.Stats(profiler, stream=s).sort_stats('cumulative')
    ps.print_stats(30)
    logger.warning("[CPROFILE]\n%s", s.getvalue())

    return response


def _server_view_impl(request):
    with profile_queries("PAGE_COMPLETE"):
        ...
        with profile_queries("user_filters"):
            base_queryset = apply_filters_to_queryset(base_queryset, filters, su_fields=su_fields)
            _ = base_queryset.exists()

        with profile_queries("annotation_filter"):
        ...
        with profile_queries("sort_pagination_setup"):
            # Sort handling
            ...
            for field_name, field_info in json_data_fields.items():
                model_fields.append({
                    ...
                    'is_visible': (
                        visible_columns is None
                        or field_name == 'SERVER_ID'
                        or field_name in visible_columns
                    ),
                })

        with profile_queries("hostnames_pagination_query"):
            #  Data fetching
        ...
        with profile_queries("main_data_fetch"):
        ...
        with profile_queries("serverunique_enrich"):
        ...
        with profile_queries("page_obj_and_stats"):
        ...
        with profile_queries("listbox_load"):
        ...
        with profile_queries("table_fields_build"):
        ...
        with profile_queries("category_fields_build"):
        ...
        with profile_queries("other_context"):
            # Other context data
        ...
        with profile_queries("render_template"):
...
def get_filtered_servers(requestfilters, permanent_filter_selection):
    ...
    for key, value in filters.items():
        if key.endswith('__gte') or key.endswith('__lte'):
            field_name, lookup = key.rsplit('__', 1)
            try:
                d = datetime.datetime.strptime(value[0], '%Y-%m-%d').date()
            except (ValueError, TypeError):
                combined_filter_query &= Q(**{field_name: value[0]})
                continue
            servers = _apply_char_date_filter(servers, field_name, lookup, d.isoformat())
        else:
    ...
    # Apply sort from filters (used by exports; views use request.GET directly)
    sort_field = requestfilters.get('sort', 'SERVER_ID')
    sort_order = requestfilters.get('order', 'asc')
    valid_sort_fields = {f for f in json_data.get('fields', {}).keys()} - su_fields - {'ANNOTATION'}
    if sort_field not in valid_sort_fields:
        sort_field = 'SERVER_ID'
    order_expr = F(sort_field).asc(nulls_last=True) if sort_order == 'asc' else F(sort_field).desc(nulls_last=True)
    servers = servers.order_by(order_expr, 'SERVER_ID')
...
def export_to_file(request, filetype):
    ...
    # Build the (lazy, unevaluated) filtered queryset only — every DB read,
    # including the metadata lookups below, happens inside background_export()
    # so the request returns job_id immediately instead of blocking on them.
    servers = get_filtered_servers(requestfilters, permanent_filter_selection)
    ...
    logger.info(
        f"[Export] {request.user.username or 'anonymous'} — flat {filetype} — "
        f"path={request.path} — job_id={job_id} — columns={columns} — filters={requestfilters}"
    )
    ...
    def background_export():
        try:
            with _EXPORT_SEMAPHORE:
                hostname_list = list(servers.values_list('SERVER_ID', flat=True).distinct())

                annotations_dict = {}
                if exportnotes and hostname_list:
                    annotations_dict = {
                        ann.SERVER_ID: ann.notes or ''
                        for chunk in _chunked(hostname_list, QUERY_CHUNK_SIZE)
                        for ann in ServerAnnotation.objects.filter(SERVER_ID__in=chunk)
                    }
                ...
                su_columns = [c for c in columns if c in su_fields]
                su_dict = {}
                if su_columns and hostname_list:
                    su_dict = {
                        su.hostname: su
                        for chunk in _chunked(hostname_list, QUERY_CHUNK_SIZE)
                        for su in ServerUnique.objects.filter(hostname__in=chunk)
                    }
...
def export_to_file_grouped(request, filetype):
    ...
    # Build the (lazy, unevaluated) filtered queryset only — every DB read
    # happens inside background_export() so the request returns job_id
    # immediately. Ordered by SERVER_ID so rows for the same server are
    # contiguous — the grouped export streams this ordering instead of
    # loading every Server row into a dict upfront.
    servers = get_filtered_servers(requestfilters, permanent_filter_selection).order_by('SERVER_ID')

    job_id = str(uuid.uuid4())  # Generate a unique identifier
    filepath = os.path.join(EXPORT_DIR, f"{job_id}.{filetype}")

    logger.info(
        f"[Export] {request.user.username or 'anonymous'} — grouped {filetype} — "
        f"path={request.path} — job_id={job_id} — columns={columns} — filters={requestfilters}"
    )
            ...
            with _EXPORT_SEMAPHORE:
                ...
                if hostname_list:
                    summaries_dict = {
                        s.SERVER_ID: s
                        for chunk in _chunked(hostname_list, QUERY_CHUNK_SIZE)
                        for s in ServerGroupSummary.objects.filter(SERVER_ID__in=chunk)
                    }
                ...
                if filetype == 'xlsx':
                    generate_excel_grouped(filepath, servers, summaries_dict, annotations_dict, columns, su_dict=su_dict, su_fields=su_fields)
                else:
                    generate_csv_grouped(filepath, servers, summaries_dict, annotations_dict, columns, su_dict=su_dict, su_fields=su_fields)
        except Exception as e:
            print(f"Error during the export : {e}")
        finally:
            close_old_connections()
...
def edit_annotation(request, hostname):
    ...
    if request.method == 'GET':
        ...
        if action in ('resolve', 'reopen'):
            if not annotation:
                return JsonResponse({'success': False, 'message': 'No annotation for this server'}, status=404)

            entry_date = request.POST.get('entry_date', '')
            changed = annotation.resolve_entry(entry_date) if action == 'resolve' else annotation.reopen_entry(entry_date)
        ...
        # Adding a new entry (annotations are otherwise append-only).
...
# Handles the bulk import of server annotations from a CSV file
@require_http_methods(["GET", "POST"])
def bulk_import_csv(request):
    ...
    if reader.fieldnames != base_headers:
        errors = [
    ...
    def run():
        try:
            ...
            # Servers can be typed with any casing — resolve everything against
            # the canonical SERVER_ID casing stored in the inventory (one bulk query).
            hostnames_upper = {
                (row.get('hostname') or '').strip().upper()
                for row in rows_data
                if (row.get('hostname') or '').strip()
            }

            update_job(job_id, 10, 'Loading servers from inventory...')
            server_map = {}  # hostname upper -> canonical SERVER_ID casing
            for sid_upper, sid in (
                Server.objects
                    .annotate(sid_upper=Upper('SERVER_ID'))
                    .filter(sid_upper__in=hostnames_upper)
                    .values_list('sid_upper', 'SERVER_ID')
                    .distinct()
            ):
                server_map.setdefault(sid_upper, sid)

            update_job(job_id, 20, 'Loading existing annotations...')
            annotation_map = {}  # hostname upper -> existing ServerAnnotation instance
            for annotation in (
                ServerAnnotation.objects
                    .annotate(sid_upper=Upper('SERVER_ID'))
                    .filter(sid_upper__in=hostnames_upper)
            ):
                annotation_map[annotation.sid_upper] = annotation

            to_create = {}  # hostname upper -> new ServerAnnotation (not yet saved)
            to_update = {}  # hostname upper -> existing ServerAnnotation (mutated)
            ignored_count = 0
            errors = []
            ...
            for i, row in enumerate(rows_data):
                ...
                canonical = server_map.get(upper)
                ...
                xtype = (row.get('type', "") or "").upper() or None
                servicenow = row.get('servicenow', "") or None

                instance = to_create.get(upper) or to_update.get(upper)
                if instance is None:
                    instance = annotation_map.get(upper)
                    if instance is None:
                        instance = ServerAnnotation(SERVER_ID=canonical, history=[])
                        to_create[upper] = instance
                    else:
                        to_update[upper] = instance
                ...
                if servicenow is not None:
                    instance.servicenow = servicenow
                ...
                now = time.monotonic()
                if now - last_progress_update >= 0.5 or i == total_lines - 1:
                    progress = 20 + ((i + 1) / total_lines) * 50  # 20% -> 70%
                    update_job(job_id, progress, f'Reading CSV… {i + 1}/{total_lines}')
                    last_progress_update = now

            created_list = list(to_create.values())
            updated_list = list(to_update.values())

            update_job(job_id, 70, f'Creating {len(created_list)} new annotations...')
            if created_list:
                ServerAnnotation.objects.bulk_create(created_list, batch_size=100)

            update_job(job_id, 85, f'Saving {len(updated_list)} updated annotations...')
            for i in range(0, len(updated_list), 100):
                batch = updated_list[i:i + 100]
                ServerAnnotation.objects.bulk_update(batch, ['notes', 'type', 'servicenow', 'history'])
                progress = 85 + ((i + len(batch)) / max(len(updated_list), 1)) * 15
                update_job(job_id, progress, f'Saving… {i + len(batch)}/{len(updated_list)}')

            updated_count = len(created_list) + len(updated_list)
            finish_job(job_id, stats={
                ...
                'message': (
                    ...
                    f"({len(created_list)} new), {ignored_count} ignored."
...
def _resolve_canonical_server_ids(raw_ids):
    """Case-insensitive resolution of SERVER_IDs against real inventory rows."""
    from django.db.models.functions import Lower
    input_lowers = {(rid or '').strip().lower() for rid in raw_ids if (rid or '').strip()}
    return {
        sid.lower(): sid
...
def api_bulk_annotation(request):
    ...
    PUT  /inventory/api/annotations/  -> upsert (append) annotations for multiple servers
    POST /inventory/api/annotations/  -> fetch full annotation history for multiple servers
    ...
    PUT response: { "success": true, "created": N, "updated": N, "errors": [...] }

    POST body: JSON array of SERVER_IDs
      ["SERVER_A", "SERVER_B"]
    POST response:
      {
        "success": true,
        "annotations": {"SERVER_A": [{"text": ..., "type": ..., "servicenow": ..., "status": "open"|"resolved", "date": ..., "user": ...}, ...], ...},
        "not_found": ["SERVER_C"]
      }
    """
    ...
    if request.method == 'POST':
        server_ids = [(sid or '').strip() for sid in data if (sid or '').strip()]
        canonical_map = _resolve_canonical_server_ids(server_ids)
        not_found = [sid for sid in server_ids if sid.lower() not in canonical_map]

        annotations = ServerAnnotation.objects.filter(SERVER_ID__in=set(canonical_map.values()))
        result = {
            ann.SERVER_ID: ann.get_history_display()
            for ann in annotations
        }
    ...
    canonical_map = _resolve_canonical_server_ids(item.get('SERVER_ID') or '' for item in data)

    # Group by canonical SERVER_ID first: the payload can list the same server
    # more than once, and every one of those entries must be kept.
    entries_by_server = {}
    for item in data:
        ...
        entries_by_server.setdefault(canonical_id, []).append({
    ...
    existing = {
        ann.SERVER_ID: ann
        for ann in ServerAnnotation.objects.filter(SERVER_ID__in=entries_by_server.keys())
    }
    ...
    for server_id, entries in entries_by_server.items():
        annotation = existing.get(server_id)
        if annotation is None:
            annotation = ServerAnnotation(SERVER_ID=server_id, history=[])
            instances_to_create.append(annotation)
            created_count += 1
        else:
            instances_to_update.append(annotation)
            updated_count += 1

        annotation.history = (annotation.history or []) + entries
        last_entry = entries[-1]
        annotation.notes = last_entry['text']
        annotation.type = last_entry['type']
        annotation.servicenow = last_entry['servicenow']

    if instances_to_create:
        ServerAnnotation.objects.bulk_create(instances_to_create)
...
def history_query(request):
    ...
    if not has_perm(request.user, 'inventory.history'):
    ...
    first_row  = ServerHistory.objects.order_by('valid_from').values('valid_from').first()
    first_date = first_row['valid_from'] if first_row else None
...
def history_drilldown(request):
    ...
    if not has_perm(request.user, 'inventory.history'):
        return JsonResponse({'error': 'Access denied'}, status=403)
    from datetime import date as _date
    from inventory.history_query_engine import get_meta, drilldown_servers, _DRILLDOWN_LIMIT
    ...
    result = drilldown_servers(point_date, group_by, group_values, filters,
                               measure_field=measure_field, measure_agg=measure_agg,
                               limit=0 if export else _DRILLDOWN_LIMIT)
    return JsonResponse(result)


@login_required
def history_diff(request):
    """Compare two inventory snapshots and return added/removed/changed servers."""
    if not has_perm(request.user, 'inventory.history'):
        return JsonResponse({'error': 'Access denied'}, status=403)
    from datetime import date as _date
    from inventory.history_query_engine import compare_snapshots

    try:
        date_a = _date.fromisoformat(request.GET.get('date_a', ''))
        date_b = _date.fromisoformat(request.GET.get('date_b', ''))
    except (ValueError, TypeError):
        return JsonResponse({'error': 'Invalid dates'}, status=400)

    if date_a >= date_b:
        return JsonResponse({'error': 'date_a must be before date_b'}, status=400)

    export = request.GET.get('export') == '1'
    result = compare_snapshots(date_a, date_b, limit=0 if export else 500)
    return JsonResponse(result)
