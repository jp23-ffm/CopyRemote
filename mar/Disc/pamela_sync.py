...
def _pamela_tool_labels():
    # {tool_code: full_name} for the sidebar legend — derived from pamela_report_queries.json's
    # "comment" strings (e.g. "Active Directory - To exclude staging machines, add ..."), same
    # single-source-of-truth principle as PAMELA_TOOL_CHOICES. Only the part before the first
    # " - " is the tool's full name, the rest is an unrelated SQL note.
    queries = load_pamela_report_queries()
    labels = {}
    for tool in PAMELA_TOOL_CHOICES:
        comment = queries.get(f'missing_{tool}', {}).get('comment', '')
        labels[tool] = comment.split(' - ')[0].strip() or tool
    return labels


PAMELA_TOOL_LABELS = _pamela_tool_labels()
