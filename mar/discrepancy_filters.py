...
@register.simple_tag
def severity_class(pct_issue, orange, red):
    # Recap table color: always driven by the % missing/inconsistent, even when rendering
    # the "clean" side of the table — same underlying cell, so both sides must agree on color.
    # pct_issue is '' (not None) when the template variable resolution itself failed (e.g.
    # cell.percentage_with_issues where cell is None) — treat that the same as missing.
    if pct_issue is None or pct_issue == '':
        return ''
    pct_issue = float(pct_issue)
    if pct_issue >= red:
        return 'sev-critical'
    if pct_issue > orange:
        return 'sev-warning'
