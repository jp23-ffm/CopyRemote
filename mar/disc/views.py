...
def _build_cross_matrix(snapshot):
    # Region (rows) x OS-bucket (columns) matrix, plus its transpose — both orientations
    # are cheap to build from the same rows and let the page show "by region" and "by OS" views.
    cross_rows = list(snapshot.cross_breakdowns.all())
    ...
    regions = sorted({r.region for r in cross_rows})

    by_region = {region: {} for region in regions}
    by_bucket = {bucket: {} for bucket in buckets}
    for row in cross_rows:
        by_region.setdefault(row.region, {})[row.os_bucket] = row
        by_bucket.setdefault(row.os_bucket, {})[row.region] = row

    return {'regions': regions, 'buckets': buckets, 'by_region': by_region, 'by_bucket': by_bucket}


def _resolve_snapshot(date_param):
    # Resolve the most recent AnalysisSnapshot on or before the requested date.
    # Falls back to the latest snapshot if no date was requested.
...
def historic_breakdown_view(request):
    ...
    snapshot, requested_date = _resolve_snapshot(date_param)
    ...
    alive_panels = _breakdown_panels_for_metric(snapshot, AnalysisSnapshotBreakdown.METRIC_ALIVE_INCONSISTENT)
    dead_panels = _breakdown_panels_for_metric(snapshot, AnalysisSnapshotBreakdown.METRIC_DEAD_INCONSISTENT)

    cross_matrix = _build_cross_matrix(snapshot)
    ...
    context = {
        ...
        'alive_panels': alive_panels,
        'dead_panels': dead_panels,
...
def historic_breakdown_export(request):
    ...
    date_param = request.GET.get('date', '').strip()
    snapshot, _ = _resolve_snapshot(date_param)
    ...
    add_dimension_sheets(AnalysisSnapshotBreakdown.METRIC_ALIVE_INCONSISTENT, prefix="Alive ")
    add_dimension_sheets(AnalysisSnapshotBreakdown.METRIC_DEAD_INCONSISTENT, prefix="Dead ")

    # ── Region x OS matrix ───────────────────────────────────────────────
    cross_matrix = _build_cross_matrix(snapshot)
    if cross_matrix['regions'] and cross_matrix['buckets']:
        ws = wb.create_sheet("Region x OS")
