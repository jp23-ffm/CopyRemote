    ...
    for widget in config['dashboard']['widgets']:
        ...
        if widget['type'] == 'gauge':
            if widget['size'] == 'large':
                ...
                if widget.get('physical_only', 'no') == 'yes':
                    total = latest_snapshot.total_physical_servers
                else:
                    total = latest_snapshot.total_servers_analyzed
                servers_ok = total - metric_value
                
                if total >0:
                    percentage_ok = round((servers_ok / total) * 100, 4) # Calucluate with enough decimals
                    percentage_ok = math.trunc(percentage_ok * 100) / 100 # Truncate to have the 2 relevant decimals without rounding
                else:
                    percentage_ok = 100
                ...
                if percentage_ok == 100:
                    widget_data['display_value'] = "100%"
...
def dashboard_filter_api(request):
    ...
    # --- "All Servers": return snapshot data directly ---
    if not filter_name or filter_name not in permanent_filters:
        try:
            snapshot = AnalysisSnapshot.objects.latest('analysis_date')
        except AnalysisSnapshot.DoesNotExist:
            return JsonResponse({'error': 'No snapshot available'}, status=404)

        metrics = {metric: getattr(snapshot, metric, 0) for metric in FIELD_TO_METRIC.values()}
        metrics['alive_status_inconsistent_count'] = snapshot.alive_status_inconsistent_count
        metrics['dead_status_inconsistent_count']  = snapshot.dead_status_inconsistent_count

        return JsonResponse({
            'total_servers':   snapshot.total_servers_analyzed,
            'total_physical':  snapshot.total_physical_servers,
            'total_discrepant': snapshot.servers_with_issues,
            'servers_clean':   snapshot.servers_clean,
            'percentage_clean': snapshot.percentage_clean,
            'metrics': metrics,
            'filter_link_params': '',
        })

    # --- Active filter: live counts from ServerDiscrepancy ---
    filter_def = permanent_filters[filter_name]   # e.g. {"OSFAMILY": ["WINDOWS"]}

    disc_q = Q()
    inv_q  = Q()
    link_parts = []
    for field, values in filter_def.items():
        disc_q &= Q(**{f'{field}__in': values})
        inv_q  &= Q(**{f'{field}__in': values})
        inputname = field_to_inputname.get(field, field.lower())
        for v in values:
            link_parts.append(f'{inputname}={v}')

    excluded_names = set(ExcludedServer.objects.values_list('server_name', flat=True))

    disc_qs = ServerDiscrepancy.objects.filter(disc_q)
    if excluded_names:
        disc_qs = disc_qs.exclude(SERVER_ID__in=excluded_names)
    total_discrepant = disc_qs.count()

    from inventory.models import Server as InventoryServer
    base_q = Q(LIVE_STATUS='ALIVE', SNOW_STATUS='OPERATIONAL', INFRAVERSION__in=['IV1', 'IV2', 'IBM'])
    inv_qs = InventoryServer.objects.filter(base_q & inv_q)
    if excluded_names:
        inv_qs = inv_qs.exclude(SERVER_ID__in=excluded_names)
    total_servers  = inv_qs.count()
    total_physical = inv_qs.filter(MACHINE_TYPE='PHYSICAL').count()

    metrics = {}
    for field, metric_name in FIELD_TO_METRIC.items():
        # Use missing_fields column, not the field value — mirrors how AnalysisSnapshot
        # counts are built (discrepancies_by_field). Status-inconsistent servers have
        # missing_fields='' so they're naturally excluded from field quality counts.
    ...
    servers_clean = max(0, total_servers - total_discrepant)
    pct = math.trunc((servers_clean / total_servers) * 10000) / 100 if total_servers > 0 else 100
    ...
    if request.method == 'POST':
        ...
        for name in valid_names:
            ExcludedServer.objects.create(
                server_name=name.upper(),           # keep user‑provided spelling
                reason=reason,
                owner=owner,
