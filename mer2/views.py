...
def dashboard_export_excel(request):
    ...
    if pf_name and pf_name in permanent_filters:
        for field, values in permanent_filters[pf_name].items():
            inv_q  &= Q(**{f'{field}__in': values})
            disc_q &= Q(**{f'{field}__in': values})

    disc_base = ServerDiscrepancy.objects.filter(disc_q) if disc_q else ServerDiscrepancy.objects.all()
    if excluded_names:
        disc_base = disc_base.exclude(SERVER_ID__in=excluded_names)

    inv_eligible = InventoryServer.objects.filter(
        Q(LIVE_STATUS='ALIVE', SNOW_STATUS='OPERATIONAL', INFRAVERSION__in=['IV1', 'IV2', 'IBM']) & inv_q
    ) if inv_q else InventoryServer.objects.filter(
        LIVE_STATUS='ALIVE', SNOW_STATUS='OPERATIONAL', INFRAVERSION__in=['IV1', 'IV2', 'IBM']
    )
    if excluded_names:
        inv_eligible = inv_eligible.exclude(SERVER_ID__in=excluded_names)
    ...
    # Title row
    filter_label = pf_name if pf_name and pf_name in permanent_filters else 'All Servers'
