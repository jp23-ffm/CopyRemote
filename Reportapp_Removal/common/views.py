...
from userapp.models import SavedChart, UserProfile
...
def custom_login(request):
    if request.method == 'POST':
        ...
        app_selection = request.POST.get('app_selection', 'inventory')

        user = authenticate(request, username=username, password=password)
        if user is not None:
            login(request, user)
            if app_selection == 'businesscontinuity':
                return redirect('businesscontinuity:server_view')
            else:
                return redirect('inventory:server_view')
        else:
...
def initiate_sso(request):
    ...
    app_selection = request.GET.get('app', 'inventory')
    if app_selection == 'businesscontinuity':
        next_url = "/businesscontinuity/servers/"
    else:
        next_url = "/inventory/servers/"
