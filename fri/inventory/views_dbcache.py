...
# EXPERIMENTAL: DB-backed cache (shared across gunicorn workers/processes),
# used only for listbox values. See batch_load_listbox_values() below.
# Compare against inventory/views.py, which uses the default LocMemCache.
...
def batch_load_listbox_values(listbox_fields, su_fields=None, force_refresh=False):
    ...
    for field in listbox_fields:
        ...
        if not force_refresh:
            ...
            if cached:  # treat an empty list as a miss, not a cached truth
                result[field] = cached
                continue
    ...
    with connection.cursor() as cursor:
        for field in fields_to_query:
            ...
            # No TTL: data only changes once a day at import time, so this is
            # invalidated explicitly (force_refresh=True) rather than guessed.
            # Don't persist an empty result — it would mean the source table
            # was momentarily empty (e.g. mid-import) and would otherwise be
            # cached as "correct" forever.
            if values:
                listbox_cache.set(f'{cache_key_prefix}{field}', values, None)
