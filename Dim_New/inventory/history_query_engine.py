...
def generate_dates(start, end, step):
    if step == 'week':
        # Anchor to end date so the last point is always end, go back in 7-day steps
        dates, d = [], end
        while d >= start:
            dates.append(d)
            d -= timedelta(weeks=1)
...
def _drilldown_fields(group_by, measure_field, measure_agg):
    ...
    count_distinct + non-SERVER_ID primary: return [primary] only — deduplicated entity list.
    count_distinct + SERVER_ID: return SERVER_ID + group dims for context.
    sum: return SERVER_ID + group dims + measure_field.
    """
    if measure_agg == 'count_distinct' and measure_field != 'SERVER_ID':
...
def drilldown_servers(point_date, group_by, group_values, filters,
    ...
    primary = fields[0]
    total = qs.values(primary).distinct().count()
    qs_ordered = qs.values(*fields).distinct().order_by(primary)
...
def compare_snapshots(date_a, date_b, limit=_COMPARE_LIMIT):
    ...
    changed       = []
    changed_count = 0
    transitions   = defaultdict(int)   # (field, from, to) → server count, always uncapped
    ...
    for sid in sorted(ids_a & ids_b):
        ...
        if diffs:
            ...
            for d in diffs:
                transitions[(d['field'], d['from'], d['to'])] += 1

    transition_rows = sorted(
        [{'field': f, 'from': fv, 'to': tv, 'count': cnt}
         for (f, fv, tv), cnt in transitions.items()],
        key=lambda x: (x['field'], -x['count'])
    )
