...
let currentChartsBaseUrl = null;
let chartsLoaded = false;
let savedCharts = []; // Store loaded charts

function initChartModal(jsonData, appName, chartsBaseUrl) {
    availableFields = jsonData.fields || {};
    availableCategories = jsonData.categories || {};
    currentAppName = appName;
    currentChartsBaseUrl = chartsBaseUrl || null;
}
...
function updateChartTypes(chartNumber) {
    ...
    const suitableCharts = ['pie', 'doughnut', 'bar', 'hbar', 'line'];
    const chartIcons = { 'pie': '🥧', 'doughnut': '🍩', 'bar': '📊', 'hbar': '☰', 'line': '📈' };
    const chartLabels = { 'pie': 'Pie Chart', 'doughnut': 'Donut Chart', 'bar': 'Bar Chart', 'hbar': 'Horizontal Bar', 'line': 'Line Chart' };
...
function loadSavedChart(chartId) {
    ...
    const base = currentChartsBaseUrl || `/${currentAppName}/charts/`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (chartForm) {
        chartForm.addEventListener('submit', function(e) {
            ...
            const base = currentChartsBaseUrl || `/${currentAppName}/charts/`;
            const url = `${base}?` + currentParams.toString();
            window.open(url, '_blank');
