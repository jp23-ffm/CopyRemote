...
def export_to_file(request, filetype):
    ...
    def background_export():
        try:
            if filetype == 'xlsx':
                generate_excel(filepath, servers, annotations_dict, columns, FILTER_MAPPING, exportnotes, su_dict=su_dict, su_fields=su_fields)
            else:
                generate_csv(filepath, servers, annotations_dict, columns, FILTER_MAPPING, exportnotes, su_dict=su_dict, su_fields=su_fields)
...
def export_to_file_grouped(request, filetype):
    ...
    def background_export():
        try:
            if filetype == 'xlsx':
                generate_excel_grouped(filepath, hostnames, server_groups, summaries_dict, annotations_dict, columns, exportnotes, su_dict=su_dict, su_fields=su_fields)
            else:
                generate_csv_grouped(filepath, hostnames, server_groups, summaries_dict, annotations_dict, columns, exportnotes, su_dict=su_dict, su_fields=su_fields)
        except Exception as e:
            print(f"Error during the export : {e}")
...
def server_relations(request, hostname):
    ...
    # ── Property relations without parent (direct or grouped) ────────────────
    for rel in config.get('relations', []):
        if rel['type'] != 'property' or rel.get('parent'):
            continue
        field  = rel['field']
    ...
    # ── Property relations WITH parent (per-instance pairing) ────────────────
    for rel in config.get('relations', []):
        if rel['type'] != 'property' or not rel.get('parent'):
            continue
        field        = rel['field']
        parent_field = rel['parent']
        meta         = field_meta.get(field, {})
        parent_meta  = field_meta.get(parent_field, {})
        if not meta.get('values') or not parent_meta.get('values'):
            continue

        for v in meta['values']:
            node_id     = f'prop__{field}__{v}'
            v_instances = meta['idx_map'][v]
            add_node({'id': node_id, 'label': v, 'field_label': rel['label'],
                      'type': 'property', 'color': rel.get('color', 'property'),
                      'field': field, 'value': v})

            # Link to every parent value that shares at least one instance
            linked = False
            for pv, pv_instances in parent_meta['idx_map'].items():
                if v_instances & pv_instances:
                    parent_node_id = f'prop__{parent_field}__{pv}'
                    if parent_node_id in seen_node_ids:
                        add_link(parent_node_id, node_id)
                        linked = True
            # Fallback: if parent nodes were not created (e.g. parent had 1 value,
            # direct-linked) make sure the link still exists
            if not linked:
                add_link(hostname, node_id)
