...
class ServerAnnotation(models.Model):
    ...
    def get_history_display(self):
        ...
        normalized = [{**entry, 'status': entry.get('status', 'open')} for entry in self.history]
        return sorted(normalized, key=lambda x: x['date'], reverse=True)

    def _set_entry_status(self, entry_date, status):
    ...
    def resolve_entry(self, entry_date):
        return self._set_entry_status(entry_date, 'resolved')

    def reopen_entry(self, entry_date):
        return self._set_entry_status(entry_date, 'open')

    def get_open_count(self):
        return sum(1 for entry in (self.history or []) if entry.get('status', 'open') == 'open')

    def get_last_open_entry(self):
        open_entries = [e for e in (self.history or []) if e.get('status', 'open') == 'open']
        if not open_entries:
            return None
        return sorted(open_entries, key=lambda x: x['date'], reverse=True)[0]
