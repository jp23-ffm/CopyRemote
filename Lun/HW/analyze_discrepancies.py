...
HARDWARE_FIELDS_TO_CHECK = [
    'IDRAC_NAME',
    'IDRAC_IP',
]

ALL_CHECKED_FIELDS = FIELDS_TO_CHECK + HARDWARE_FIELDS_TO_CHECK
...
def load_exclusions():
    """
    Load excluded SERVER_IDs from exclusions.csv at BASE_DIR.
    Returns a set of SERVER_IDs to exclude from analysis.
    If the file doesn't exist, logs a warning and returns an empty set.
    """
    csv_path = settings.BASE_DIR / 'exclusions.csv'

    if not csv_path.exists():
        write_log("WARNING: exclusions.csv not found — no servers excluded")
        return set()
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            if 'SERVER_ID' not in (reader.fieldnames or []):
                write_log("WARNING: exclusions.csv has no SERVER_ID column — no servers excluded")
                return set()

            exclusions = {row['SERVER_ID'].strip() for row in reader if row.get('SERVER_ID', '').strip()}

        write_log(f"Loaded {len(exclusions)} exclusion(s) from exclusions.csv")
        return exclusions

    except Exception as e:
        write_log(f"WARNING: Error reading exclusions.csv: {e} — no servers excluded")
        return set()
...
def check_missing_fields(servers_with_issues, exclusions,
                         fields=None, extra_filter=None, label=""):
    """
    Check for missing/invalid field values.
    By default checks FIELDS_TO_CHECK on ALIVE servers.
    Can be called with custom fields and extra queryset filters
    (e.g. hardware-only fields on PHYSICAL servers).
    """
    if fields is None:
        fields = FIELDS_TO_CHECK

    check_label = label or "Missing fields on ALIVE servers"
    write_log(f"Check: {check_label}")

    fields_to_fetch = ['SERVER_ID'] + fields
    ...
    queryset = queryset.order_by('SERVER_ID')
...
def check_live_status_inconsistent(servers_with_issues, exclusions):
    """
...
def check_snow_status_inconsistent(servers_with_issues, exclusions):
    """
...
def check_missing_hardware_fields(servers_with_issues, exclusions):
    """Check hardware-specific fields on PHYSICAL servers only."""
    return check_missing_fields(
        servers_with_issues, exclusions,
        fields=HARDWARE_FIELDS_TO_CHECK,
        extra_filter={'MACHINE_TYPE__iexact': 'PHYSICAL'},
        label="Hardware fields on PHYSICAL servers"
    )
...
def analyze_servers():
    ...
    for check_func in ALL_CHECKS:
        count = check_func(servers_with_issues, exclusions)
        total_entries += count
...
def bulk_insert_discrepancies(records):
    ...
    columns = ['SERVER_ID', 'missing_fields', 'analysis_date'] + ALL_CHECKED_FIELDS
