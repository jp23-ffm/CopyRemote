...
def get_field_labels():
    ...
    # Vérifier si le cache est encore valide (sans lock pour la perf)
    if _field_labels_cache is not None and (current_time - _field_labels_timestamp) < CACHE_TTL:
        return _field_labels_cache

    # Cache expiré ou inexistant → recharger avec lock
    with _cache_lock:
        # Double-check après avoir acquis le lock (autre thread a pu le faire)
        if _field_labels_cache is not None and (current_time - _field_labels_timestamp) < CACHE_TTL:
            return _field_labels_cache
...
def server_view(request):
    ...
    model_fields = []
    excluded_fields = set() 
    excluded_fields.add('live_status_snow_status_inconsistent')
