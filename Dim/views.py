...
def history_query(request):
    ...
    from inventory.history_query_engine import get_meta, get_filter_values

    meta = get_meta()
    field_values = {}
    for key, field_def in meta['fields'].items():
        if field_def.get('filterable'):
            vals = get_filter_values(key)
            if vals:
...
def history_export(request):
    ...
    headers = ['Date'] + [ds['label'] for ds in data['datasets']]
    ws.append(headers)
