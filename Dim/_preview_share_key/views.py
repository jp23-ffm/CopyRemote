...
def _table_view(request, model_cls, annotation_cls, annotation_key_field,
                ...
                primary_field='SHARE_NAME'):  # CHANGED: added primary_field param
    ...
    if annotation_terms:
        ...
        queryset = queryset.filter(**{f'{primary_field}__in': matching_ids})  # CHANGED

    # Sort
    sort_field = request.GET.get('sort', primary_field)  # CHANGED: default sort = primary_field
    sort_order = request.GET.get('order', 'asc')
    valid_sort_fields = set(json_data.get('fields', {}).keys())
    if sort_field not in valid_sort_fields:
        sort_field = primary_field  # CHANGED
    order_expr = F(sort_field).asc(nulls_last=True) if sort_order == 'asc' else F(sort_field).desc(nulls_last=True)
    ...
    for field_name, field_info in json_data['fields'].items():
        model_fields.append({
            ...
            'is_hostname': field_name == primary_field,  # CHANGED: was == 'ID'
        })
    ...
    # CHANGED: hostname now maps to SHARE_NAME (via primary_field), not ID
    display_items = [{'hostname': getattr(item, primary_field), 'primary_server': item} for item in current_page_items]
...
def storage_view(request):
    return _table_view(
        ...
        annotation_key_field='SHARE_NAME',  # CHANGED: was 'SHARE_ID'
        ...
        primary_field='SHARE_NAME',  # CHANGED
    )
...
def objects_view(request):
    return _table_view(
        ...
        primary_field='ID',  # objects unchanged
    )
...
def edit_annotation(request, resource_type, resource_id):
    if resource_type == 'share':
        annotation_cls, key_field = ShareAnnotation, 'SHARE_NAME'  # CHANGED: was 'SHARE_ID'
...
def bulk_annotation(request, resource_type):
    ...
    if resource_type == 'share':
        model_cls, annotation_cls, key_field = StorageShare, ShareAnnotation, 'SHARE_NAME'  # CHANGED
        get_labels = get_share_field_labels
        item_key = 'SHARE_NAME'  # CHANGED: was 'ID'
    elif resource_type == 'object':
    ...
    try:
        ...
        def update_with_progress():
            ...
            for i in range(total_batches):
                ...
                for item in batch:
                    ann, _ = annotation_cls.objects.get_or_create(**{key_field: getattr(item, item_key)})  # CHANGED
