    ...
    history.forEach(entry => {
        ...
        div.innerHTML = `
            <div class="history-meta">
                ...
                <button class="annotation-resolve-btn" data-entry-date="${escapeHtml(entry.date)}" data-target-status="${isResolved ? 'reopen' : 'resolve'}" onclick="toggleEntryStatus(this)">
...
function toggleEntryStatus(btn) {
    if (!currentHostname) return;

    const entryDate = btn.getAttribute('data-entry-date');
    const action = btn.getAttribute('data-target-status'); // 'resolve' or 'reopen'
    btn.disabled = true;

    fetch(`/inventory/annotation/${encodeURIComponent(currentHostname)}/`, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded', 'X-CSRFToken': getCsrfToken()},
        body: `action=${action}&entry_date=${encodeURIComponent(entryDate)}`
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
