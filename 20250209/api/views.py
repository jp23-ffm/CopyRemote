...
class SrvPropView(APIView):
    ...
    Wildcards with negation:
    {
        "filters": {"HOSTNAME": ["!*TEST*", "!*DEV*"]}
    }
    """
    ...
    def post(self, request):
        try:
            ...
            if annotation_in_filters:
                ...
                exclude_values = [v[1:] for v in annotation_values if v.startswith('!')]  # Remove ! prefix
            ...
            # Exclude servers with matching annotations (negation with !)
            ...
            for field, values in mapped_filters.items():
                ...
                exclude_values = [v[1:] for v in values if v.startswith('!')]  # Remove the ! prefix
            ...
            queryset = queryset.order_by(*distinct_fields) 
