...
def server_view(request):
    ...
    # Note: ANNOTATION is kept in json_data_fields so the right-click modal can display it.
    # The table column is hidden via template logic ({% if field.name == 'ANNOTATION' and not edit_mode %}).
    ...
    finalfields = [(field, info) for field, info in json_data_fields.items() if not (field == 'ANNOTATION' and not edit_mode)]  # Read all items in the json fields section
    ...
    # Enrich with ServerUnique data (only if BC columns are visible)
    su_fields = {k for k, v in json_data_fields.items() if v.get('model_extra') == 'serverunique'}
    needs_su = visible_columns and any(f in visible_columns for f in su_fields)

    su_dict = {}
    if needs_su and hostnames_in_page:
        su_start = time.time()
        serveruniques = ServerUnique.objects.filter(hostname__in=hostnames_in_page)
        su_dict = {su.hostname: su for su in serveruniques}

        for server_group in display_servers:
            server_group['serverunique'] = su_dict.get(server_group['hostname'])

        print(f"[TIMING] ServerUnique for current page ({len(su_dict)} matches): {time.time() - su_start:.3f}s")
    ...
    if exportnotes:
        if hostname_list:
            ...
            annotations_dict = {
                ...
                for ann in annotations.values('SERVER_ID', 'notes')
            }
