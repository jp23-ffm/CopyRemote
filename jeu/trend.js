...
async function updateTrendChart(metric, color, label) {
    try {
        ...
        // Fetch new trend data from API. TREND_API_URL lets a page point this at its own trend
        // endpoint (e.g. the Pamela dashboard) instead of the default Discrepancies one.
        const days = (typeof TREND_DAYS !== 'undefined') ? TREND_DAYS : 90;
        const apiUrl = (typeof TREND_API_URL !== 'undefined') ? TREND_API_URL : '/discrepancies/api/trend/';
        const response = await fetch(`${apiUrl}?metric=${metric}&days=${days}`);
