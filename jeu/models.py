...
class ServerDiscrepancyPamela(models.Model):
    """
    One row per server flagged by PAMELA as missing at least one tool's coverage
    (AD/ADDM/SA/LA/EPO/CA), consolidated from the raw per-report_type/host rows PAMELA
    exports (same shape as ServerDiscrepancy: one row per server, missing_fields is a
    comma-separated list — here of missing report-type suffixes, e.g. "AD,LA" — instead
    of one row per (report_type, host)). techfamily/area are PAMELA's own raw values,
    same source as DailyPamelaDBSummary. No days_open/tracking: same same-day-snapshot
    convention as DailyPamelaDBSummary, not tracked as a persistent issue over time.
    """

    SERVER_ID = models.CharField(max_length=200, db_index=True)
    snapshot_date = models.DateField()
    ...
    class Meta:
        ...
        unique_together = ('SERVER_ID', 'snapshot_date')
        indexes = [
            models.Index(fields=['snapshot_date']),
            models.Index(fields=['SERVER_ID', 'snapshot_date']),
        ]

    def __str__(self):
        return f"{self.SERVER_ID} - {self.snapshot_date} - {self.missing_fields}"
