...
def generate_excel(filepath, servers, annotations_dict, columns, FILTER_MAPPING, su_dict=None, su_fields=None):
    ...
    if servers.exists():
        for server in servers:
            row = []
            first_value = get_db_attr(server, FILTER_MAPPING[first_column])
            row.append(first_value)
            for col in columns[1:]:
                if col == 'ANNOTATION':
                    value = clean_value(annotations_dict.get(server.SERVER_ID, ''))
...
def generate_csv(filepath, servers, annotations_dict, columns, FILTER_MAPPING, su_dict=None, su_fields=None):
    ...
    with open(temp_filepath, 'w', newline='', encoding='utf-8') as file:
        ...
        for server in servers:
            ...
            for col in columns[1:]:
                if col == 'ANNOTATION':
                    data_row.append(clean_value(annotations_dict.get(server.SERVER_ID, '')))
                elif col in su_fields:
                    su = su_dict.get(server.SERVER_ID) if su_dict else None
...
def generate_csv_grouped(filepath, hostnames, server_groups, summaries_dict, annotations_dict, columns, su_dict=None, su_fields=None):
...
def generate_excel_grouped(filepath, hostnames, server_groups, summaries_dict, annotations_dict, columns, su_dict=None, su_fields=None):
