...
from .models import AnalysisSnapshot, AnalysisSnapshotBreakdown, ServerDiscrepancy, DiscrepancyTracking, DiscrepancyAnnotation, ImportStatus, ExcludedServer
...
def _breakdown_panels_for_metric(snapshot, metric):
    ...
    for dimension, label in _dimension_labels():
        rows = list(snapshot.breakdowns.filter(metric=metric, dimension=dimension).order_by('dimension_value'))
        if rows:
            panels.append({'dimension': dimension, 'label': label, 'rows': rows})
    return panels
...
def _build_cross_matrix(snapshot, metric, row_field='REGION'):
    ...
    return {'regions': regions, 'buckets': buckets, 'by_region': by_region}


def _population_total(cross_matrix):
    # Sum of total_servers across every cell of a cross matrix — the real eligible
    # population for that metric (e.g. ALIVE + IV1/IV2/IBM for alive_inconsistent).
    return sum(
        cell.total_servers
        for buckets in cross_matrix['by_region'].values()
...
def _build_recap_table(snapshot):
    ...
    rows = [
        {'field': field, 'label': label, 'cells': by_field[field]}
        for field, label in row_fields if field in by_field
    ]
...
def _resolve_snapshot(date_param, snapshot_id_param=None):
    ...
    if snapshot_id_param:
        try:
            return AnalysisSnapshot.objects.get(pk=int(snapshot_id_param)), requested_date
...
def historic_breakdown_view(request):
    ...
    alive_population_total = _population_total(alive_cross_matrix)
    dead_population_total = _population_total(dead_cross_matrix)
    alive_pct_clean = round(100 - (snapshot.persistent_alive_inconsistent_count / alive_population_total * 100), 2) if alive_population_total else 100
    dead_pct_clean = round(100 - (snapshot.persistent_dead_inconsistent_count / dead_population_total * 100), 2) if dead_population_total else 100
