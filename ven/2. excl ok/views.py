...
def _breakdown_panels_for_metric(snapshot, metric):
    ...
    global_population = (
        ...
        else snapshot.total_all_servers
    )
    panels = []
...
def _build_cross_matrix(snapshot, metric, row_field='REGION'):
    ...
    # AnalysisSnapshot.total_all_servers, or a genuinely empty fleet — so the template can
...
def _pct_pair(issue_count, total):
    ...
    AnalysisSnapshot.total_all_servers; older snapshots predate that field).
    """
    if not total:
        return None, None
...
def historic_breakdown_view(request):
    ...
    # Denominator is the WHOLE fleet (IV1/IV2/IBM), not the ALIVE-only/OPERATIONAL-only
    # population used to compute the inconsistencies themselves — same population the live
    # dashboard uses for its combined "Operational Inconsistencies" gauge. Falls back to the
    ...
    alive_card = _region_scoped_card(alive_cross_matrix, selected_region, snapshot.persistent_alive_inconsistent_count, snapshot.total_all_servers)
    dead_card = _region_scoped_card(dead_cross_matrix, selected_region, snapshot.persistent_dead_inconsistent_count, snapshot.total_all_servers)
...
def dashboard_export_excel(request):
    ...
    inv_all = InventoryServer.objects.filter(inv_q) if is_filtered else InventoryServer.objects.filter(INFRAVERSION__in=['IV1', 'IV2', 'IBM'])
    if excluded_names:
        inv_all = inv_all.exclude(SERVER_ID__in=excluded_names)
    total_all = inv_all.values('SERVER_ID').distinct().count()
