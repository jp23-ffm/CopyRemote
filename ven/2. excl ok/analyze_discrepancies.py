...
# Whole fleet, regardless of LIVE_STATUS/SNOW_STATUS — same population as
# AnalysisSnapshot.total_all_servers and the live dashboard's "Operational Inconsistencies"
# gauge. The alive/dead REGION x OS cross-breakdown's own total_servers/servers_clean must use
# THIS as its population, not POPULATION_FILTERS['alive_inconsistent'/'dead_inconsistent']
# (ALIVE-only/OPERATIONAL-only) — those narrower filters correctly scope which servers CAN be
# flagged alive/dead-inconsistent (numerator), but the % denominator for these two metrics is
# "all servers", same as total_all_servers already is at the snapshot level.
...
def analyze_servers(excluded_ids):
    ...
    # Whole fleet in scope, regardless of LIVE_STATUS/SNOW_STATUS — same population the live
    # dashboard uses for its "Operational Inconsistencies" gauge. This is the denominator for
    # alive/dead inconsistency percentages (AnalysisSnapshot.total_all_servers), NOT the
    # ALIVE-only/OPERATIONAL-only populations used to compute the inconsistencies themselves.
    total_all_servers = (
...
def compute_cross_breakdown(issue_ids, row_field, bucket_field, group_def, population_filter, excluded_ids):
    """
    row_field x bucket_field matrix (e.g. REGION x OS-bucket, or MANUFACTURER x OS-bucket
    for the recap table). Population totals AND issue counts are both queried directly from
    inventory.Server — this works for ANY Server field as row_field, not just ones captured
    in FIELDS_TO_CHECK (e.g. DATACENTER, which isn't checked for missing-ness but can still
    be used as a recap row dimension).
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for metric, records in metric_records.items():
                ...
                # total_servers_analyzed) — alive/dead use the whole fleet instead (matches
                # total_all_servers), see FLEET_POPULATION_FILTER above.
                ...
                for row_field, bucket_field in CROSS_BREAKDOWNS:
                    ...
                    if not group_def:
                        ...
                        continue
                    matrix = compute_cross_breakdown(issue_ids, row_field, bucket_field, group_def, cross_population_filter, excluded_ids)
