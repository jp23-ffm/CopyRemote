...
def update_tracker(records, analysis_date):
    """
    Updates DiscrepancyTracker (1 row per server, active_issues JSONField).
    ...
    existing_trackers = {t.SERVER_ID: t for t in DiscrepancyTracker.objects.all()}
    ...
    # 4. Apply changes
    if to_create:
        DiscrepancyTracker.objects.bulk_create(to_create, batch_size=1000)
        write_log(f"Tracker: created {len(to_create)} new entries")

    if to_update:
        DiscrepancyTracker.objects.bulk_update(
            to_update, ['active_issues', 'oldest_first_seen'], batch_size=1000
        )
        write_log(f"Tracker: updated {len(to_update)} entries")

    if to_delete_ids:
        DiscrepancyTracker.objects.filter(pk__in=to_delete_ids).delete()
