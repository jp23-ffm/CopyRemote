...
from .models import AnalysisSnapshot, ServerDiscrepancy, DiscrepancyTracking, DiscrepancyAnnotation, ExcludedServer
...
def load_exclusions_data():
    """
    Load the full exclusions CSV for dashboard display.
    Returns dict with headers, rows, and count.
    """
    csv_path = os.path.join(settings.BASE_DIR, 'exclusions.csv')
...
def export_status(request, job_id, filetype):
    ...
    filepath = os.path.join(EXPORT_DIR, f'{job_id}.{extension}')
...
def download_export(request, job_id, filetype):
    ...
    response = FileResponse(open(filepath, 'rb'), as_attachment=True, filename=f'export_discrepancies.{extension}')

    def delete_file_after_close(resp):
        try:
            os.remove(filepath)
        except Exception as e:
            print(f"Error removing export file: {e}")
...
def exclusion_list_api(request):
    ...
    if request.method == 'POST':
        ...
        if not server_names_raw:
            return JsonResponse({'success': False, 'message': 'Server name is required'}, status=400)
        ...
        from inventory.models import Server as InventoryServer
        existing = set(
            InventoryServer.objects.filter(SERVER_ID__in=server_names)
                                   .values_list('SERVER_ID', flat=True)
        )
        valid_names = [n for n in server_names if n in existing]
        not_found   = [n for n in server_names if n not in existing]
        ...
        return JsonResponse({'success': True, 'created': len(valid_names), 'not_found': not_found})
...
def exclusion_export_csv(request):
    ...
    writer.writerow(['Server Name', 'Reason', 'Owner', 'Excluded Until', 'Added By', 'Added At'])
...
def exclusion_export_excel(request):
    ...
    headers = ['Server Name', 'Reason', 'Owner', 'Excluded Until', 'Added By', 'Added At', 'Status']
    ws.append(headers)
