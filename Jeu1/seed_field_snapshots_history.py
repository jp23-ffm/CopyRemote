...
from inventory.models import FieldSnapshotFiltered as FieldSnapshot
    ...
    def handle(self, *args, **options):
        ...
        for day_offset in range(1, n_days + 1):
            ...
            for snap in today_snapshots:
                ...
                new_counts = {
                    fv: max(0, round(count * factor))
                    for fv, count in snap.counts.items()
                }

                to_create.append(FieldSnapshot(
                    date=target_date,
                    field_name=snap.field_name,
                    filter_field=snap.filter_field,
                    filter_value=snap.filter_value,
                    filter_field2=snap.filter_field2,
                    filter_value2=snap.filter_value2,
                    filter_field3=snap.filter_field3,
                    filter_value3=snap.filter_value3,
                    counts=new_counts,
                ))
