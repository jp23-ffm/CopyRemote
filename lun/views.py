...
def _pamela_bucket_for(techfamily, group_def):
    # Maps DailyPamelaDBSummary.techfamily (LINUX/WINDOWS/VMWARE) onto the SAME OSFAMILY
    # buckets the rest of the recap table uses, so these rows share the table's columns
    # instead of needing their own. Case-insensitive, same convention as analyze_discrepancies'
    # bucket_for(): WINDOWS/LINUX match their bucket directly, VMWARE (not an OS family) falls
    # into the configured "Other" bucket.
    if not techfamily:
        return 'MISSING'
...
def _build_recap_table(snapshot, region=''):
    ...
    group_def = _load_breakdown_groups_config().get('groups', {}).get(bucket_field, {})
    ...
    # stored for these metrics (they were already split by region, unlike RECAP rows above).
    for metric, label in (
    ...
    # docstring), region-scoped the same way as everything else in this table. DailyPamelaDBSummary
    # only has a missing-tool COUNT, no population of its own, so it borrows the OSFAMILY row's
    # per-bucket total_servers (computed above) as the "clean" denominator — same population,
    # different missing-ness question. techfamily (LINUX/WINDOWS/VMWARE) is mapped onto the
    # table's existing OSFAMILY buckets (_pamela_bucket_for) so these rows share its columns.
    osfamily_population = {
        bucket: cell.total_servers for bucket, cell in by_field.get('OSFAMILY', {}).items()
    }
    NA_CELL = {
        'servers_with_issues': None, 'servers_clean': None,
    ...
    if region:
        pamela_qs = pamela_qs.filter(area=region)

    pamela_by_report_type = defaultdict(lambda: defaultdict(int))
    for p in pamela_qs:
        bucket = _pamela_bucket_for(p.techfamily, group_def)
        pamela_by_report_type[p.report_type][bucket] += p.total_count

    for report_type, label in _pamela_report_types():
        by_bucket = pamela_by_report_type.get(report_type)
        if not by_bucket:
        ...
        cells = {
            bucket: {
                'servers_with_issues': servers_with_issues,
                'servers_clean': max(0, osfamily_population.get(bucket, 0) - servers_with_issues),
                'percentage_with_issues': safe_percentage(servers_with_issues, osfamily_population.get(bucket, 0)),
                'percentage_clean': safe_percentage_clean(
                    max(0, osfamily_population.get(bucket, 0) - servers_with_issues), osfamily_population.get(bucket, 0)
                ),
            }
            for bucket, servers_with_issues in by_bucket.items()
        }
        total_servers = sum(osfamily_population.get(bucket, 0) for bucket in by_bucket)
    ...
    zero_cell = {
        'servers_with_issues': 0, 'servers_clean': 0,
...
def _xl_add_recap_sheet(wb, styles, recap_table, sheet_name, count_attr, pct_attr):
    # count_attr/pct_attr pick which side of the recap table to export — servers_with_issues/
    # percentage_with_issues for the normal recap, servers_clean/percentage_clean for its mirror.
    ...
    for row in recap_table['rows']:
        ...
        for bucket in recap_table['buckets']:
            ...
            row_cells.append(_xl_cell_get(cell, count_attr))
            row_cells.append(_xl_cell_get(cell, pct_attr))
            pct_issues.append(_xl_cell_get(cell, 'percentage_with_issues'))
        ws.append(row_cells)
        r = ws.max_row
        ws.cell(row=r, column=1).font = styles['bold']
        for i, bucket in enumerate(recap_table['buckets']):
            count_col = 2 + i * 2
            pct_col = count_col + 1
            ws.cell(row=r, column=count_col).alignment = styles['right']
            pct_cell = ws.cell(row=r, column=pct_col)
            pct_cell.number_format = '0.00"%"'
            pct_cell.alignment = styles['right']
            # None on Pamela rows with no data for this date/region (N/A in the HTML table) —
            # leave unfilled rather than crash comparing None >= red.
            if pct_issues[i] is not None:
                pct_cell.fill = _xl_recap_severity_fill(pct_issues[i], severity_orange, severity_red, styles)
    _xl_autosize(ws, [20] + [12, 12] * len(recap_table['buckets']))
...
def historic_breakdown_export(request):
    ...
    # ── Recap table (missing_data + Alive/Dead rows, all-regions aggregate) ────────────────
    recap_table = _build_recap_table(snapshot)
    _xl_add_recap_sheet(wb, styles, recap_table, "Recap", 'servers_with_issues', 'percentage_with_issues')
    _xl_add_recap_sheet(wb, styles, recap_table, "Recap Clean", 'servers_clean', 'percentage_clean')
...
def historic_breakdown_export_recap(request):
    # Lighter export than historic_breakdown_export: just the two recap sheets (missing data
    # recap + its clean mirror), no per-dimension/cross-tab sheets — for when management only
    # wants the recap numbers, not the full workbook.
