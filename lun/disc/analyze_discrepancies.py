...
def create_analysis_snapshot(stats, analysis_date, duration, diff=None, persistent_records=None, persistent_days_threshold=7):
    field_mapping = {
...
def compute_breakdowns(records, dimension_fields):
    """
    Aggregate issue counts by dimension value (REGION, OSSHORTNAME) for the current run.
    Population counts (total_servers) come from the same eligible population as the
    main analysis (ALIVE / OPERATIONAL / IV1+IV2+IBM).
    ...
    for dimension in dimension_fields:
        ...
        for record in records:
            ...
            for check_name in inconsistency_names:
                if record.get(check_name) == VALIDATION_KO:
                    slot['field_counts'][check_name] += 1
...
def save_breakdowns(snapshot, breakdowns):
    rows = [
    ...
    if rows:
        ...
        write_log(f"Saved {len(rows)} breakdown rows")
...
def compute_cross_breakdown(records, row_field, bucket_field, group_def):
    """
    row_field x bucket_field matrix (e.g. REGION x OS-bucket). Population totals come
    from the eligible Server population; issue counts from the given (already
    persistent-filtered) records.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            persistent_records = filter_persistent_records(stats['records'], persistent_days)
            write_log(f"Persistent issues (>= {persistent_days}d open): {len(persistent_records)} / {len(stats['records'])} raw")
            ...
            snapshot = create_analysis_snapshot(
                ...
                persistent_records=persistent_records, persistent_days_threshold=persistent_days,
            )

            group_config = load_breakdown_groups()
            dimension_fields = breakdown_dimension_fields(group_config)

            breakdowns = compute_breakdowns(persistent_records, dimension_fields)
            save_breakdowns(snapshot, breakdowns)
            ...
            for row_field, bucket_field in CROSS_BREAKDOWNS:
                ...
                if not group_def:
                    ...
                    continue
                matrix = compute_cross_breakdown(persistent_records, row_field, bucket_field, group_def)
