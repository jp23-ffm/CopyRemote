...
def historic_breakdown_view(request):
    ...
    for dimension, label in _dimension_labels():
        rows = list(snapshot.breakdowns.filter(dimension=dimension).order_by('dimension_value'))
        if rows:
            breakdown_panels.append({'dimension': dimension, 'label': label, 'rows': rows})
...
def historic_breakdown_export(request):
    # Excel export of a historic snapshot: one tab per dimension (Region, OS) + a Summary tab.
    ...
    summary_ws.append(["Total servers analyzed", snapshot.total_servers_analyzed])
    summary_ws.append(["Servers with persistent issues", snapshot.persistent_servers_with_issues])
    summary_ws.append(["Servers clean", snapshot.persistent_servers_clean])
    summary_ws.append(["% with persistent issues", snapshot.percentage_persistent_issues])

    for dimension, sheet_name in _dimension_labels():
        rows = list(snapshot.breakdowns.filter(dimension=dimension).order_by('dimension_value'))
        if not rows:
            continue
