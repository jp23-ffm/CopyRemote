...
from .models import AnalysisSnapshot, ServerDiscrepancy, DiscrepancyTracker
...
def server_view(request):
    ...
    if sort_field == 'days_open':
        ...
        oldest_issue = DiscrepancyTracker.objects.filter(
            SERVER_ID=OuterRef('SERVER_ID'),
            is_resolved=False
        ).order_by('first_seen').values('first_seen')[:1]
        all_servers = all_servers.annotate(oldest_issue=Subquery(oldest_issue))
    ...
    if hostnames_in_page:
        trackers = DiscrepancyTracker.objects.filter(
            SERVER_ID__in=hostnames_in_page,
            is_resolved=False
        )
        tracker_data = {}
        for t in trackers:
            if t.SERVER_ID not in tracker_data or t.days_open > tracker_data[t.SERVER_ID]['days_open']:
                tracker_data[t.SERVER_ID] = {
                    'days_open': t.days_open,
                    'first_seen': t.first_seen,
                    'issues_count': 0,
                }
            tracker_data[t.SERVER_ID]['issues_count'] += 1

        for server_group in display_servers:
            td = tracker_data.get(server_group['hostname'])
            server_group['tracker'] = td
            if td:
                server_group['primary_server'].days_open = td['days_open']
            else:
                server_group['primary_server'].days_open = ''
...
def dashboard_view(request):
    ...
    exclusions = load_exclusions_data()
