...
def safe_percentage(count, total):
    ...
    read as "no issues"), it floors to 0.01 instead. Symmetrically, a count short of the
    full total never rounds UP to a literal 100.0 (3 issues out of a large population
    must not silently read as "100% clean" on the flip side — see safe_percentage_clean),
    it caps at 99.99 instead. Numeric, not a display string: callers include Excel
    severity-fill thresholds (pct >= 20 etc.) that compare it.
    """
    if not total:
        return 0
...
def safe_percentage_clean(clean_count, total):
    """
    Like safe_percentage, but for the "clean" side: an empty population (0 servers) has
    zero issues by definition, so it reads as 100% clean, not 0%.
    """
    if not total:
        return 100
    return safe_percentage(clean_count, total)
...
class ServerDiscrepancy(models.Model):
    ...
    SNOW_DATACENTER = models.CharField(max_length=100, blank=True, null=True)
...
class AnalysisSnapshot(models.Model):
    ...
    # TRUE fleet size in scope (IV1/IV2/IBM), regardless of LIVE_STATUS/SNOW_STATUS — includes
    # servers that were properly decommissioned and are correctly inactive. Kept for audit/
    # reporting, but NOT used as the alive/dead % denominator anymore (see
    # total_relevant_servers below) — with ~80% of the fleet legitimately retired, dividing by
    # this real total drowns out the inconsistency % into meaninglessness. Snapshots created
    # before this field existed have it at 0.
    total_all_servers = models.IntegerField(default=0)

    # Alive/dead inconsistency % denominator: total_servers_analyzed (the ALIVE+OPERATIONAL
    # active population) + the inconsistencies themselves (persistent_alive_inconsistent_count +
    # persistent_dead_inconsistent_count) — NOT total_all_servers. An alive/dead-inconsistent
    # server is by definition outside the active population, so adding the two together gives
    # "everything that's either properly active or a known anomaly", deliberately excluding the
    # bulk of properly-decommissioned servers that aren't relevant to this analysis. Snapshots
    # created before this field existed have it at 0 (same fallback as total_all_servers).
    total_relevant_servers = models.IntegerField(default=0)
...
class AnalysisSnapshotBreakdown(models.Model):
    ...
    def percentage_with_issues(self):
        ...
        total = (
            ...
            else self.snapshot.total_relevant_servers
        )
...
class AnalysisSnapshotCrossBreakdown(models.Model):
    ...
    def percentage_clean(self):
        ...
        # as a literal 0% either, same as servers_with_issues doesn't. An empty population
        # (0 servers) is 100% clean by definition, not 0% — see safe_percentage_clean.
        return safe_percentage_clean(self.servers_clean, self.total_servers)
...
class DailyPamelaDBSummary(models.Model):
    """
    Daily snapshot of missing-tool counts pulled from the PAMELA SQL Server DB
    (report_type = missing_AD/ADDM/SA/LA/EPO/CA), grouped by techfamily x area.
    In PROD, populated via db_utils.py (pyodbc). In DEV, populated from CSV exports
    since there's no direct DB access. No days_open: this is a same-day snapshot,
    replaced/kept per snapshot_date, not tracked as a persistent issue over time.
    """

    report_type = models.CharField(max_length=50)
    techfamily = models.CharField(max_length=100)
    ...
    class Meta:
        db_table = 'discrepancies_dailypameladbsummary'
        # This prevents duplicate rows for the same techfamily/area/type/date
        unique_together = ('report_type', 'techfamily', 'area', 'snapshot_date')
        ordering = ['-snapshot_date', 'report_type']

    def __str__(self):
        return f"{self.snapshot_date} | {self.report_type} | {self.techfamily} | {self.area}: {self.total_count}"


class ServerDiscrepancyPamela(models.Model):
    """
    One row per server currently flagged by PAMELA as missing at least one tool's coverage
    (AD/ADDM/SA/LA/EPO/CA) — current-state table, wholesale replaced on every
    analyze_pamela_discrepancies run, same convention as ServerDiscrepancy (no history here:
    see PamelaDiscrepancyTracking for days_open and PamelaAnalysisSnapshot for the trend
    history). techfamily/area are PAMELA's own raw values, same source as DailyPamelaDBSummary.
    """

    SERVER_ID = models.CharField(max_length=200, unique=True)

    techfamily = models.CharField(max_length=100, blank=True, null=True)
    area = models.CharField(max_length=100, blank=True, null=True)

    # Comma-separated list of missing report types, e.g. "AD,LA"
    missing_fields = models.TextField(blank=True)

    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        db_table = 'discrepancies_serverdiscrepancypamela'

    def __str__(self):
        return f"{self.SERVER_ID} - {self.missing_fields}"

    @property
    def missing_fields_list(self):
        return [t for t in self.missing_fields.split(',') if t]


class PamelaDiscrepancyTracking(models.Model):
    """
    Tracks active missing-tool issues per server for PAMELA — mirrors DiscrepancyTracking
    exactly. One row per server. active_issues stores per-tool first_seen timestamps; when a
    tool's coverage is restored it is simply removed from active_issues. days_open is derived
    via the same compute_days_open() helper the classic Discrepancies view already uses.
    """

    SERVER_ID = models.CharField(max_length=200)

    # {tool_code: {"first_seen": "<ISO datetime>"}}
    active_issues = models.JSONField(default=dict)
    ...
    class Meta:
        db_table = 'discrepancies_pameladiscrepancytracking'
        constraints = [
            models.UniqueConstraint(fields=['SERVER_ID'], name='pameladisctracking_serverid_uniq'),
        ]
        indexes = [
            models.Index(fields=['oldest_first_seen'], name='pameladisctracking_oldest_idx'),
        ]
...
class PamelaAnalysisSnapshot(models.Model):
    """
    Summary statistics for each analyze_pamela_discrepancies run — simplified PAMELA
    equivalent of AnalysisSnapshot (no breakdowns/cross-breakdowns, just what the Quality
    Trend chart needs). One row per run.
    """

    analysis_date = models.DateTimeField(unique=True, db_index=True)

    servers_with_any_missing = models.IntegerField(default=0)

    # {tool_code: count}, e.g. {"AD": 21, "ADDM": 14, ...}
    tool_counts = models.JSONField(default=dict)

    class Meta:
        db_table = 'discrepancies_pamelaanalysissnapshot'
        ordering = ['-analysis_date']

    def __str__(self):
        return f"{self.analysis_date} - {self.servers_with_any_missing} with issues"


class PamelaImportStatus(models.Model):
    # Mirrors ImportStatus exactly — kept as a separate model/table so a failed Pamela run
    # never mixes into the classic Discrepancies import log (and vice versa).
    date_import = models.DateTimeField(auto_now_add=True)
    success = models.BooleanField(default=False)
    message = models.TextField(blank=True, null=True)
    nb_entries_created = models.IntegerField(default=0)
