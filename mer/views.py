...
from .models import AnalysisSnapshot, AnalysisSnapshotBreakdown, ServerDiscrepancy, DiscrepancyTracking, DiscrepancyAnnotation, ImportStatus, ExcludedServer, safe_percentage
    ...
    for field, label in row_fields:
        ...
        rows.append({
            ...
            'total': {
                ...
                'percentage_clean': safe_percentage(servers_clean, total_servers),
            },
        })
    ...
    for metric, label in (
        ...
        cells = {
            bucket: {
                ...
                'percentage_clean': safe_percentage(data['total_servers'] - data['servers_with_issues'], data['total_servers']),
            }
...
def _xl_add_recap_sheet(wb, styles, recap_table, sheet_name, count_attr, pct_attr):
    ...
    for row in recap_table['rows']:
        ...
        for i, bucket in enumerate(recap_table['buckets']):
            ...
            pct_cell.fill = _xl_severity_fill(row_cells[pct_col - 1], styles)
...
def historic_breakdown_export(request):
    ...
    def add_cross_sheet(metric, sheet_name):
        ...
        for region in cross_matrix['regions']:
            ...
            r = ws.max_row
            ws.cell(row=r, column=1).font = styles['bold']
            for col in range(2, len(cross_matrix['buckets']) + 2):
                ws.cell(row=r, column=col).alignment = styles['right']
