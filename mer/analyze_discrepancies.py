...
from discrepancies.models import ServerDiscrepancy, AnalysisSnapshot, DiscrepancyTracker
...
def update_tracker(records, analysis_date):
    """
    Met à jour DiscrepancyTracker avec les résultats de l'analyse.
    - Nouveaux problèmes → INSERT avec first_seen=now
    - Problèmes existants → UPDATE last_seen=now, is_resolved=False (au cas où re-ouvert)
    - Problèmes non retrouvés → marquer is_resolved=True, resolved_date=now
    """

    # 1. Construire le set des problèmes actuels : {(SERVER_ID, field_name), ...}
    current_issues = set()
    for record in records:
        ...
        if missing:
            for field_name in missing.split(','):
                ...
                if field_name:
                    current_issues.add((record['SERVER_ID'], field_name))

        if record.get('live_status_inconsistent') == 'KO':
            current_issues.add((record['SERVER_ID'], 'live_status_inconsistent'))

    write_log(f"Tracker: {len(current_issues)} active issues to track")

    # 2. Charger les entrées ouvertes existantes
    open_trackers = DiscrepancyTracker.objects.filter(is_resolved=False)
    existing_open = {(t.SERVER_ID, t.field_name): t for t in open_trackers}

    # 3. Traiter les problèmes actuels
    to_create = []
    to_update_last_seen = []

    for server_id, field_name in current_issues:
        key = (server_id, field_name)
        if key in existing_open:
            tracker = existing_open[key]
            tracker.last_seen = analysis_date
            to_update_last_seen.append(tracker)
            del existing_open[key]
        else:
            to_create.append(DiscrepancyTracker(
                SERVER_ID=server_id,
    ...
    # 4. Ce qui reste dans existing_open = problèmes résolus
    to_resolve = list(existing_open.values())

    # 5. Appliquer les changements
    if to_update_last_seen:
        DiscrepancyTracker.objects.bulk_update(to_update_last_seen, ['last_seen'], batch_size=1000)
        write_log(f"Tracker: updated last_seen for {len(to_update_last_seen)} entries")

    if to_create:
        # Gérer le cas de réouverture : si la combinaison existe (resolved), on la réouvre
        reopened = 0
        truly_new = []
        existing_resolved = {
            (t.SERVER_ID, t.field_name): t
            for t in DiscrepancyTracker.objects.filter(
                is_resolved=True,
                SERVER_ID__in=[t.SERVER_ID for t in to_create],
            )
        }
        for tracker in to_create:
            key = (tracker.SERVER_ID, tracker.field_name)
            if key in existing_resolved:
                old = existing_resolved[key]
                old.first_seen = analysis_date
                old.last_seen = analysis_date
                old.is_resolved = False
                old.resolved_date = None
                old.save()
                reopened += 1
            else:
                truly_new.append(tracker)

        if truly_new:
            DiscrepancyTracker.objects.bulk_create(truly_new, batch_size=1000)
        write_log(f"Tracker: created {len(truly_new)} new, reopened {reopened}")

    if to_resolve:
        for tracker in to_resolve:
            tracker.is_resolved = True
            tracker.resolved_date = analysis_date
        DiscrepancyTracker.objects.bulk_update(to_resolve, ['is_resolved', 'resolved_date'], batch_size=1000)
        write_log(f"Tracker: resolved {len(to_resolve)} entries")
