...
def compute_cross_breakdown(issue_ids, row_field, bucket_field, group_def, population_filter):
    """
...
def compute_recap_breakdown(records, fields, bucket_field, group_def, population_filter, region_field='REGION'):
    """
    ...
    if issue_ids:
        for row in Server.objects.filter(SERVER_ID__in=issue_ids, **population_filter).values('SERVER_ID', bucket_field, region_field):
            region_value = row[region_field] if is_value_valid(row[region_field]) else MISSING_MARKER
        ...
        try:
            ...
            for metric, records in metric_records.items():
                ...
                if mode == 'dimensions':
                    breakdowns = compute_breakdowns(
                        records, dimension_fields, population_filter,
                ...
                for row_field, bucket_field in CROSS_BREAKDOWNS:
                    ...
                    if not group_def:
                        ...
                        continue
                    matrix = compute_cross_breakdown(issue_ids, row_field, bucket_field, group_def, cross_population_filter)
            ...
            if recap_fields and recap_group_def:
                matrix = compute_recap_breakdown(
                    missing_data_persistent, recap_fields, recap_bucket_field, recap_group_def, POPULATION_FILTERS['missing_data']
                )
