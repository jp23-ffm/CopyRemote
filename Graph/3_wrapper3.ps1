...
<#
.SYNOPSIS
    Répond à un email via Graph (équivalent à EmailMessage.Reply)
    
.DESCRIPTION
    Simule la méthode Reply() d'EWS en envoyant une réponse via Microsoft Graph
    
.PARAMETER MessageId
    ID du message auquel répondre (peut être l'objet message complet ou juste l'ID)
    
.PARAMETER ReplyBody
    Corps de la réponse
    
.PARAMETER ReplyToAll
    Si true, répond à tous les destinataires (défaut: false)
    
.EXAMPLE
    Send-EwsCompatReply -MessageId $message.Id -ReplyBody "Merci pour votre message" -ReplyToAll $false
    
.EXAMPLE
    Send-EwsCompatReply -MessageId $message -ReplyBody "OK" -ReplyToAll $true
#>
function Send-EwsCompatReply {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        $MessageId,
    ...
    try {
        # Extraire l'ID si c'est un objet message
        $messageIdString = $MessageId
        if ($MessageId -is [PSCustomObject] -and $MessageId.Id) {
            $messageIdString = $MessageId.Id
        }
        
        Write-Verbose "Préparation de la réponse au message: $messageIdString"
        Write-Verbose "Reply to all: $ReplyToAll"
        ...
        # Choisir entre Reply et ReplyAll selon le paramètre
        if ($ReplyToAll) {
            Write-Verbose "Envoi de la réponse à tous les destinataires"
            # Utiliser l'API ReplyAll
            Invoke-MgReplyAllUserMessage -UserId $script:EwsCompat_CurrentUser `
                                         -MessageId $messageIdString `
                                         -BodyParameter $replyMessage
        }
        else {
            Write-Verbose "Envoi de la réponse à l'expéditeur uniquement"
            # Utiliser l'API Reply
            Invoke-MgReplyUserMessage -UserId $script:EwsCompat_CurrentUser `
                                     -MessageId $messageIdString `
                                     -BodyParameter $replyMessage
        }
        
        Write-Verbose "Réponse envoyée avec succès via Graph"
        ...
        Write-Error "Erreur lors de l'envoi de la réponse: $_"
        Write-Verbose "Détails: $($_.Exception.Message)"
