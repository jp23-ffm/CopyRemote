...
def exclusion_list_api(request):
    """
    GET  – return all excluded servers as JSON.
    POST – create one or several entries (comma-separated server_names).
    """
    if request.method == 'GET':
        exclusions = ExcludedServer.objects.all()
        data = [
            {
                'id': e.id,
    ...
    if request.method == 'POST':
        ...
        return JsonResponse({'success': True, 'created': len(server_names)})
...
def exclusion_export_csv(request):
    ...
    writer.writerow(['Server Name', 'Reason', 'Owner', 'Exclusion Date', 'Added By', 'Added At'])
