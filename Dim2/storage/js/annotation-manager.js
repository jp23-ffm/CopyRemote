...
let currentAnnotationUrlBase = null;

function editAnnotation(hostname, annotationUrlBase) {
    currentHostname = hostname;
    currentAnnotationUrlBase = annotationUrlBase;

    document.getElementById('annotationModal').style.display = 'flex';
    document.getElementById('modal-hostname').value = hostname;

    fetch(`${annotationUrlBase}?key=${encodeURIComponent(hostname)}`)
...
function saveAnnotation() {
    ...
    fetch(`${currentAnnotationUrlBase}?key=${encodeURIComponent(currentHostname)}`, {
        method: 'POST',
        headers: {
...
function clearAnnotation() {
    ...
    if (!window.confirm('Are you sure you want to clear the annotation?')) return;
