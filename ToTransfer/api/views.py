...
class SrvPropView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "**ANNOTATION**: virtual field available on the `inventory` (uses SERVER_ID) and `hardware` (uses SERIAL) indexes."
        ),
    ...
    def _process_annotations(self, ctx):
        ...
        # ANNOTATION only available with inventory and hardware
        if (annotation_in_filters or annotation_in_fields) and annotation_model is None:
            raise QueryValidationError({
                "error": "ANNOTATION is only available for indexes 'inventory' and 'hardware'"
            })

        # If ANNOTATION in fields, the index ID field must also be present
        if annotation_in_fields:
            id_field = 'SERVER_ID' if index_name == 'inventory' else ('SERIAL' if index_name == 'hardware' else None)
        ...
        if include_values:
            ...
            api_log(f"ANNOTATION include filter matched {len(annotation_ids)} {'servers' if index_name == 'inventory' else 'hardware items'}")
        ...
        if exclude_values:
            ...
            api_log(f"ANNOTATION exclude filter matched {len(exclude_annotation_ids)} {'servers' if index_name == 'inventory' else 'hardware items'} to exclude")
    ...
    def _build_queryset(self, ctx):
        ...
        ctx.distinct_fields = [replace_with_alias(self.ALIAS_MAP, f) for f in ctx.distinct_fields]  # Apply the mapping if necessary
        ...
        queryset = queryset.distinct(*ctx.distinct_fields).order_by(*ctx.distinct_fields)   # Postgresql
    ...
    def _add_annotations_to_data(self, data, index_name='inventory'):
        """Add annotations to serialized data. Expects list of dicts with SERVER_ID or SERIAL."""
        if not data:
            return data
        ...
        annotations = annotation_model.objects.filter(**{f'{id_field}__in': ids})
        annotation_map = {getattr(ann, id_field): getattr(ann, notes_field) or '' for ann in annotations}
...
class QueryBuilderView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_dir = os.path.dirname(__file__)
        fields_data = {}
        for app in ('inventory', 'businesscontinuity', 'discrepancies', 'hardware'):
            labels_path = os.path.join(current_dir, f'../{app}/field_labels.json')
            try:
                ...
                fields_data[app] = {'categories': {}, 'fields': []}
