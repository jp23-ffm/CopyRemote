...
def _snapshot_qs_for_request(request, field_name):
    """Return a filtered FieldSnapshot queryset based on request date params."""
    qs = FieldSnapshot.objects.filter(field_name=field_name)
...
def field_snapshots_data(request):
    ...
    qs = _snapshot_qs_for_request(request, field_name)

    rows = list(qs.order_by('date', 'field_value').values('date', 'field_value', 'count'))
    if not rows:
    ...
    for r in rows:
        v = r['field_value']
        if v not in data_by_value:
            data_by_value[v] = {}
            value_totals[v] = 0
        data_by_value[v][r['date']] = r['count']
        value_totals[v] += r['count']
...
def field_snapshots_export(request):
    ...
    for field_name in field_names:
        rows = list(
            _snapshot_qs_for_request(request, field_name)
            .order_by('date', 'field_value')
            .values('date', 'field_value', 'count')
        )
        if not rows:
            continue
