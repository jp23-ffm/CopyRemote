...
from hardware.models import Server as HardwareServer
from storage.models import StorageShare, StorageObject
...
class EnrichSerializer(serializers.Serializer):
    index = serializers.ChoiceField(choices=["businesscontinuity", "inventory", "discrepancies"])
    fields = serializers.ListField(child=serializers.CharField(), required=True)


class SrvPropSerializer(serializers.Serializer):
    index = serializers.ChoiceField(choices=["businesscontinuity", "inventory", "discrepancies", "hardware", "storageshare", "storageobject"])
...
class AppliPropSerializer(serializers.Serializer):
    filters = serializers.DictField(required=True, child=serializers.ListField(child=serializers.CharField()))
    fields = serializers.ListField(child=serializers.CharField(), required=False)
...
class ServerDiscrepancySerializer(DynamicFieldsModelSerializer):
    class Meta:
        model = ServerDiscrepancy
        fields = '__all__'
        
class HardwareServerSerializer(DynamicFieldsModelSerializer):
    class Meta:
        model = HardwareServer
        fields = '__all__'


class StorageShareSerializer(DynamicFieldsModelSerializer):
    class Meta:
        model = StorageShare
        fields = '__all__'


class StorageObjectSerializer(DynamicFieldsModelSerializer):
    class Meta:
        model = StorageObject
        fields = '__all__'
