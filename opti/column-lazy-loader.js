...
(function() {
    ...
    function init() {
        ...
        // Setup column checkbox listeners
        setupColumnCheckboxListeners();

        // Setup category checkbox listeners
    ...
    function setupColumnCheckboxListeners() {
    ...
    async function loadColumnData(columns) {
        ...
        // Filter out already loaded columns
        ...
        // Fetch uncached data from server
        if (uncachedColumns.length > 0) {
            await fetchColumnDataFromServer(uncachedColumns);
        }
    }

    async function fetchColumnDataFromServer(columns) {
        // Mark as loading
        columns.forEach(col => state.loadingColumns.add(col));

        // Show loading indicators
        showLoadingIndicators(columns);

        try {
            // Build request params
            const params = new URLSearchParams({
                columns: columns.join(','),
                hostnames: state.currentHostnames.join(','),
                view: state.viewMode,
                page: state.currentPage,
                page_size: state.pageSize,
                permanentfilter: state.permanentFilter,
                filters: JSON.stringify(state.currentFilters)
            });

            // Get CSRF token
            const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value;

            const response = await fetch(`${CONFIG.apiColumnData}?${params}`, {
                method: 'GET',
                headers: {
            ...
            console.log('[ColumnLazyLoader] Loaded columns:', columns);

        } catch (error) {
            console.error('[ColumnLazyLoader] Error loading columns:', error);

            // Mark as not loading on error
            columns.forEach(col => state.loadingColumns.delete(col));

            // Show error in cells
            showErrorInColumns(columns, error.message);
        } finally {
            hideLoadingIndicators(columns);
        }
    }
