...
def servers_bulk_update(request):
    ...
    if request.method == "POST":
        try:
            ...
            #action_during_lp = action_during_lp if action_during_lp else "EMPTY"
            original_action_during_lp = request.POST.get('original_action_during_lp', '')
            #original_action_during_lp = original_action_during_lp if original_action_during_lp else "EMPTY"
            cluster = request.POST.get('cluster', '')
            cluster = cluster.upper() if cluster else "EMPTY"
            cluster_type = request.POST.get('cluster_type', '')
            #cluster_type = cluster_type if cluster_type else "EMPTY"
                ...
                for i in range(total_batches):
                    ...
                    with transaction.atomic():
                        for obj in batch:
                            ...
                            existing_history = obj.global_history or []
                            history_entry = {
                                "date": now_date,
                                "user": now_usr,
                                "fromsource": "Bulk Edit",
                                "changes": {}
                            }

                            if priority_asset != '-':
                                history_entry["changes"]["priority_asset"] = {
                                    "old": obj.priority_asset,
                                    "new": priority_asset
                                }
                                ...
                                history_entry["changes"]["priority_asset"] = {
                                    "old": obj.priority_asset,
                                    "new": "Not set or not modified"
                                }                            

                            if in_live_play != '-':
                                history_entry["changes"]["in_live_play"] = {
                                    "old": obj.in_live_play,
                                    "new": in_live_play
                                }
                            ...
                            if action_during_lp:
                                ...
                                if action_during_lp != "EMPTY":
                                    ...
                                    history_entry["changes"]["action_during_lp"] = {
                                        "info": "Has been edited, please check its history"
                                    }
                                else:
                                    history_entry["changes"]["action_during_lp"] = {
                            ...
                            obj.global_history = existing_history
                        ...
                        ServerUnique.objects.bulk_update(batch, ['priority_asset', 'in_live_play', 'action_during_lp', 'action_during_lp_history', 
                            'original_action_during_lp', 'original_action_during_lp_history', 'cluster', 'cluster_type', 'global_history'])
...
def _import_applications_csv(request, csv_file):
    ...
    def process_import():
        ...
        # Fetch ServerUnique instances, apply in_live_play and history
        server_unique_qs = {
            su.id: su
        ...
        for su_id, new_lp in server_lp.items():
            ...
            if su is None:
                continue
            old_lp = su.in_live_play
            su.in_live_play = new_lp
            history_entry = {
                "date": now_date,
                "user": now_usr,
                "fromsource": "CSV Application Import",
                "changes": {"in_live_play": {"old": old_lp, "new": new_lp}},
            }
            existing_history = su.global_history or []
            if isinstance(existing_history, dict):
                existing_history = [existing_history]
            existing_history.append(history_entry)
            su.global_history = existing_history
    ...
    if request.method == 'POST':  # Treat the data after the csv file has been selected
        ...
        if form.is_valid():
            ...
            def process_import():
                ...
                # Loop in every row of the csv file
                for i, row in enumerate(reader):
                    # Retrieve the hostname to update
                    hostname = row.get('hostname_unique')
                    ...
                    # If the hostname doesn't exist in the businesscontinuity_serverunique database, it is unknown and is skipped
                    try:
                        server_unique = ServerUnique.objects.get(hostname__iexact=hostname)
                        is_new_server = False
                    except ServerUnique.DoesNotExist:
                        ignored_count += 1
                    ...
                    # Check if the imported data is different from the current data
                    changes = {}
                    
                    changes['priority_asset'] = {
                        'old': server_unique.priority_asset,
                        'new': priority_asset if priority_asset is not None else 'Not set or not modified'
                    }
                    
                    changes['in_live_play'] = {
                        'old': server_unique.in_live_play,
                        'new': in_live_play if in_live_play is not None else 'Not set or not modified'
                    }
                    ...
                    changes['cluster'] = {
                        'old': server_unique.cluster,
                        'new': cluster if cluster is not None else 'Not set or not modified'
                    }

                    changes['cluster_type'] = {
                        'old': server_unique.cluster_type,
                        'new': cluster_type if cluster_type is not None else 'Not set or not modified'
                    }
                    ...
                    # Add a new entry to the global history if there are changes

                    history_entry = {
                        "date": now_date,
                        "user": now_usr,
                        "fromsource": "CSV Import",
                        "changes": changes
                    }
                    existing_history = server_unique.global_history or []
                    if isinstance(existing_history, dict):
                        existing_history = [existing_history]
                    existing_history.append(history_entry)
                    server_unique.global_history = existing_history

                    if not is_new_server:
                        instances_to_update[server_unique.id] = server_unique

                    lines_processed += 1
                    progress = (lines_processed / total_lines) * 80  # reading phase: 0% → 80%
