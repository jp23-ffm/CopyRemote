...
def _apply_char_date_filter(queryset, field_name, lookup, iso_value):
    """Add a WHERE clause comparing a dd.mm.yyyy CharField against an ISO date string."""
    if connection.vendor == 'sqlite':
        check = f'"{field_name}" GLOB \'??.??.????\''
    else:
        check = f'"{field_name}" ~ \'^\\d{{2}}\\.\\d{{2}}\\.\\d{{4}}$\''
    op = '>=' if lookup == 'gte' else '<='
    sql = (
        f'(CASE WHEN {check} THEN '
