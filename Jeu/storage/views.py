...
def _share_annotation_key(item):
    return f"{item.FILER or ''}|{item.SHARE_PATH or ''}"


def _object_annotation_key(item):
    return item.NAME or ''
...
def _get_filtered_queryset(model_cls, annotation_cls, annotation_key_field,
                           get_field_labels_fn, requestfilters, default_sort='ID'):
    ...
    sort_field = requestfilters.get('sort', default_sort)
...
def _table_view(request, model_cls, annotation_cls, annotation_key_field,
                ...
                primary_field='ID', annotation_key_fn=None):
    ...
    if annotation_terms:
        ...
        queryset = queryset.filter(**{f'{primary_field}__in': matching_ids})

    # Sort
    sort_field = request.GET.get('sort', primary_field)
    sort_order = request.GET.get('order', 'asc')
    valid_sort_fields = set(json_data.get('fields', {}).keys())
    if sort_field not in valid_sort_fields:
        sort_field = primary_field
    order_expr = F(sort_field).asc(nulls_last=True) if sort_order == 'asc' else F(sort_field).desc(nulls_last=True)
    ...
    for field_name, field_info in json_data['fields'].items():
        model_fields.append({
            ...
            'is_hostname': field_name == primary_field,
        })
    ...
    _key_fn = annotation_key_fn or (lambda item: getattr(item, primary_field))
    display_items = [{'hostname': _key_fn(item), 'primary_server': item} for item in current_page_items]
...
def storage_view(request):
    return _table_view(
        ...
        annotation_key_field='share_key',
        ...
        annotation_key_fn=_share_annotation_key,
    )
...
def objects_view(request):
    return _table_view(
        ...
        primary_field='NAME',
        annotation_key_fn=_object_annotation_key,
    )
...
@login_required
def edit_annotation(request, resource_type):
    if resource_type == 'share':
        annotation_cls, key_field = ShareAnnotation, 'share_key'
    elif resource_type == 'object':
        annotation_cls, key_field = ObjectAnnotation, 'NAME'
    else:
        return JsonResponse({'success': False, 'message': 'Invalid resource type'}, status=400)

    resource_id = (request.GET.get('key') or request.POST.get('key') or '').strip()
    if not resource_id:
...
def bulk_annotation(request, resource_type):
    ...
    if resource_type == 'share':
        model_cls, annotation_cls, key_field = StorageShare, ShareAnnotation, 'share_key'
        get_labels = get_share_field_labels
        item_key_fn = _share_annotation_key
    elif resource_type == 'object':
        model_cls, annotation_cls, key_field = StorageObject, ObjectAnnotation, 'NAME'
        get_labels = get_object_field_labels
        item_key_fn = _object_annotation_key
    else:
    ...
    try:
        ...
        def update_with_progress():
            ...
            for i in range(total_batches):
                ...
                for item in batch:
                    ann, _ = annotation_cls.objects.get_or_create(**{key_field: item_key_fn(item)})
...
def chart_view(request, resource_type):
    ...
    items = _get_filtered_queryset(model_cls, annotation_cls, key_field, get_labels, requestfilters, default_sort=default_sort)
...
def export_to_file(request, resource_type, filetype):
    ...
    if resource_type == 'share':
        ...
        get_labels = get_share_field_labels
        default_sort = 'SHARE_NAME'
        ann_key_fn = _share_annotation_key
    elif resource_type == 'object':
        model_cls, annotation_cls, key_field = StorageObject, ObjectAnnotation, 'NAME'
        get_labels = get_object_field_labels
        default_sort = 'NAME'
        ann_key_fn = _object_annotation_key
    else:
        return JsonResponse({'error': 'Invalid resource type'}, status=400)
    ...
    items_qs = _get_filtered_queryset(model_cls, annotation_cls, key_field, get_labels, requestfilters, default_sort=default_sort)
    items_list = list(items_qs)
    key_list = [ann_key_fn(item) for item in items_list]
    annotations_dict = {}
    if key_list:
        anns = annotation_cls.objects.filter(**{f'{key_field}__in': key_list})
    ...
    def background_export():
        try:
            if filetype == 'xlsx':
                generate_excel(filepath, items_list, annotations_dict, columns, annotation_key_fn=ann_key_fn)
            else:
                generate_csv(filepath, items_list, annotations_dict, columns, annotation_key_fn=ann_key_fn)
        except Exception as e:
