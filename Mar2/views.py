...
from .models import Server, ServerGroupSummary, ServerAnnotation, ImportStatus, FieldSnapshot
...
def _snapshot_qs_for_request(request, field_name, filter_field='', filter_value=''):
    """Return a filtered FieldSnapshot queryset based on request date params."""
    qs = FieldSnapshot.objects.filter(
        field_name=field_name, filter_field=filter_field, filter_value=filter_value
    )
...
def field_snapshots_dashboard(request):
    ...
    # Build {field_name: [{field, label, values}, ...]} for filter dropdowns
    filter_options: dict = {}
    for fc in fields_config:
        ...
        for fconf in fc.get('filters', []):
            ff = fconf['field']
            values = list(
                FieldSnapshot.objects
                .filter(field_name=fn, filter_field=ff)
                .values_list('filter_value', flat=True)
                .distinct()
                .order_by('filter_value')
            )
            if values:
                filter_options.setdefault(fn, []).append({
                    'field': ff,
...
def field_snapshots_data(request):
    ...
    qs = _snapshot_qs_for_request(request, field_name, filter_field, filter_value)
...
def field_snapshots_export(request):
    ...
    if request.GET.get('all') == '1':
        ...
        filename = 'field_trends_all.xlsx'
    else:
        field_name = request.GET.get('field', '').strip()
        if not field_name:
            return JsonResponse({'error': 'field parameter required'}, status=400)
        field_names = [field_name]
        filename = f"field_trends_{field_name}.xlsx"
    ...
    for field_name in field_names:
        ...
        ff = '' if len(field_names) > 1 else export_filter_field
        fv = '' if len(field_names) > 1 else export_filter_value

        rows = list(
            _snapshot_qs_for_request(request, field_name, ff, fv)
            .order_by('date')
            .values('date', 'counts')
        )
        if not rows:
        ...
        # Header row: Date + Total + one column per value
        ...
        for row_idx, date in enumerate(date_set, 2):
