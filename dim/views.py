...
def _build_recap_table(snapshot):
    # One row per configured field ("how many servers are missing this field"), split by
    # OS-bucket column. missing_data only — see the "recap" section of breakdown_groups.json.
    ...
    cross_rows = list(snapshot.cross_breakdowns.filter(
        metric=AnalysisSnapshotBreakdown.METRIC_MISSING_DATA, row_field='RECAP'
    ))
    present_buckets = {r.os_bucket for r in cross_rows}
    buckets = [b for b in bucket_order if b in present_buckets] or sorted(present_buckets)
    ...
    for field, label in row_fields:
        if field not in by_field:
            continue
        cells = by_field[field].values()
        total_servers = sum(c.total_servers for c in cells)
        servers_with_issues = sum(c.servers_with_issues for c in cells)
        rows.append({
            'field': field,
            'label': label,
...
def historic_breakdown_view(request):
    ...
    # dashboard uses for its combined "Operational Inconsistencies" gauge.
    total_all = snapshot.total_all_servers
    alive_pct_clean, alive_pct_issues = _pct_pair(snapshot.persistent_alive_inconsistent_count, total_all)
    dead_pct_clean, dead_pct_issues = _pct_pair(snapshot.persistent_dead_inconsistent_count, total_all)
    ...
    context = {
        ...
        'alive_pct_clean': alive_pct_clean,
        'dead_pct_clean': dead_pct_clean,
        'alive_pct_issues': alive_pct_issues,
        'dead_pct_issues': dead_pct_issues,
