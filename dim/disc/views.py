...
def _build_recap_table(snapshot, region=''):
    ...
    for report_type, label in _pamela_report_types():
        ...
        if report_type not in pamela_by_report_type or not pamela_population:
            ...
            # PAMELA has no data yet, as opposed to "0 servers affected". This is "no row at
            # all for this report_type" — a bucket that DOES have a report_type row elsewhere
            # but not for e.g. Windows (see below) is a real, countable zero, not N/A.
            rows.append({'label': label, 'cells': {}, 'total': NA_CELL, 'no_data': True})
            continue
    ...
    for row in rows:
        # A no_data row (see PAMELA_REPORT_TYPES above) gets N/A on every column, never the
        # 0-issues/100%-clean zero_cell — there's a real difference between "checked, found
        # nothing" and "nothing to check because there's no data for this date/region at all".
        fill = NA_CELL if row.get('no_data') else zero_cell
