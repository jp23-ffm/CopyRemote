document.addEventListener('DOMContentLoaded', function () {
    ...
    function createFilterTag(column, term) {
        ...
        // Hide system filters added automatically by dashboard links
        const HIDDEN_FILTERS = { alive_status_inconsistent: ['!KO'], dead_status_inconsistent: ['!KO'] };
        if (HIDDEN_FILTERS[column] && HIDDEN_FILTERS[column].includes(normalizedTerm)) return;
    ...
    function hasActiveFilters() {
        ...
        if (filterTagsDiv.length === 0) {
            ...
            localStorage.removeItem('selectedSearchName_' + _appname2);
        } else {
	...
	if (saveSearchForm) {
		...
		saveSearchForm.addEventListener('submit', function(event) {
        	...
        	delete filters['ss_name'];  // never persist the dashboard label param
    ...
    const urlParams = new URLSearchParams(window.location.search);
    for (const [key, value] of urlParams.entries()) {
        if (key !== 'page' && key !== 'visible_columns' && key !== 'page_size' && key !== 'scrollLeft' && key !== 'ss_name' && (!key.startsWith('cat_'))) {
            if (!filters[key]) {
                filters[key] = [];
            }
