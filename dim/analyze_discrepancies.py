...
def compute_recap_breakdown(records, fields, bucket_field, group_def, population_filter):
    """
    One row per configured field (e.g. MANUFACTURER): how many persistent missing-data
    records are missing THAT field, split by the server's OS bucket — not by the field's
    own values. The % denominator is the real eligible population per OS bucket (same
    population for every field row, queried once).

    Returns {(field, bucket_value): {'total_servers', 'servers_with_issues', 'servers_clean'}}
    """
    bucket_population = (
        Server.objects.filter(**population_filter)
        .values(bucket_field)
    ...
    for row in bucket_population:
        totals_by_bucket[bucket_for(row[bucket_field], group_def)] += row['total']

    issue_ids = [r['SERVER_ID'] for r in records]
    bucket_by_server = {}
    if issue_ids:
        for row in Server.objects.filter(SERVER_ID__in=issue_ids, **population_filter).values('SERVER_ID', bucket_field):
            bucket_by_server[row['SERVER_ID']] = bucket_for(row[bucket_field], group_def)

    counts = defaultdict(int)
    for record in records:
        bucket_value = bucket_by_server.get(record['SERVER_ID'])
        if not bucket_value:
            continue
    ...
    matrix = {}
    for field in fields:
        for bucket_value, total in totals_by_bucket.items():
            with_issues = counts.get((field, bucket_value), 0)
            matrix[(field, bucket_value)] = {
                'total_servers': total,
                'servers_with_issues': with_issues,
                'servers_clean': max(0, total - with_issues),
            }
