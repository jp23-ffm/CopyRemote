...
(function() {
    ...
    function initializeState() {
        ...
        // Identify fields from external models (e.g. ServerUnique) that require a page reload
        state.externalModelFields = new Set();
        try {
            const jsonDataEl = document.getElementById('json-data');
            if (jsonDataEl) {
                const jsonData = JSON.parse(jsonDataEl.textContent);
                for (const [fieldName, fieldInfo] of Object.entries(jsonData.fields || {})) {
                    if (fieldInfo.model_extra) {
                        state.externalModelFields.add(fieldName);
                    }
                }
            }
        } catch (e) {
            console.warn('[ColumnLazyLoader] Could not parse field labels for model_extra detection:', e);
        }
    }
    ...
    function handleColumnCheckboxChange(e) {
        ...
        // External model fields (e.g. Business Continuity) require a page reload
        if (this.checked && state.externalModelFields.has(columnName) && !state.loadedColumns.has(columnName)) {
    ...
    function handleCategoryCheckboxChange(e) {
        ...
        columnCheckboxes.forEach(colCheckbox => {
            ...
            if (this.checked) {
                showColumn(columnName);
                if (!state.loadedColumns.has(columnName) && !state.loadingColumns.has(columnName)) {
                    if (state.externalModelFields.has(columnName)) {
                        needsReload = true;
                    } else {
        ...
        // External model fields require a full page reload
        if (needsReload) {
            updateVisibleColumnsInUrl();
            window.location.search = new URL(window.location.href).search;
            return;
