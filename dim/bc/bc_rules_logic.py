...
Servers outside the datacenter are simply not included in the queryset — no action applied.
    ...
    DATACENTER (affected by DR — pre-filtered)
        └─ MACHINE_TYPE = HYPERVISOR → text I (terminal — checked first, regardless of IN_LIVE_PLAY)
...
RULE_HYPERVISOR         = "hypervisor"           # I — checked before CLUSTER, terminal
...
DEFAULT_TEXTS = {
    RULE_CLUSTER_YES_LP_YES: "A: Cluster YES, Live Play YES",
    RULE_CLUSTER_YES_LP_NO:  "B: Cluster YES, Live Play NO",
    RULE_NO_YES_PHYS:        "C: Cluster NO, Live Play YES, Machine Type PHYSICAL",
    RULE_NO_YES_VIRT_FIXED:  "D: Cluster NO, Live Play YES, Machine Type VIRTUAL, VM Type: FIXED",
    RULE_NO_YES_VIRT_STR:    "E: Cluster NO, Live Play YES, Machine Type VIRTUAL, VM Type: STRETCHED",
    RULE_NO_NO_PHYS:         "F: Cluster NO, Live Play NO, Machine Type PHYSICAL",
    RULE_NO_NO_VIRT_FIXED:   "G: Cluster NO, Live Play NO, Machine Type VIRTUAL, VM Type: FIXED",
    RULE_NO_NO_VIRT_STR:     "H: Cluster NO, Live Play NO, Machine Type VIRTUAL, VM Type: STRETCHED",
    RULE_HYPERVISOR:         "I: Machine Type HYPERVISOR",
}

# MACHINE_TYPE values that identify a physical server
PHYSICAL_MACHINE_TYPES = {"PHYSICAL", "BAREMETAL", "BARE METAL", "BARE-METAL"}

# MACHINE_TYPE value that identifies a hypervisor host — checked before CLUSTER,
# regardless of IN_LIVE_PLAY, and terminal (no further branching)
HYPERVISOR_MACHINE_TYPE = "HYPERVISOR"
...
# Sentinel returned when a server must be skipped due to undetermined field value
RULE_SKIP = None
...
def get_rule_key(
    ...
    Returns:
        One of the RULE_* constants, or None (RULE_SKIP) if the server
        has an undetermined field value (EMPTY, blank, or unknown).
        Callers must skip servers where None is returned.
    """
    # HYPERVISOR is checked first, regardless of cluster/in_live_play — terminal
    if (machine_type or "").strip().upper() == HYPERVISOR_MACHINE_TYPE:
        return RULE_HYPERVISOR

    cluster_val  = (cluster      or "").strip().upper()
    liveplay_val = (in_live_play or "").strip().upper()

    # Both fields must be explicitly YES or NO — anything else is skipped
    if cluster_val not in ("YES", "NO"):
        return RULE_SKIP
    if liveplay_val not in ("YES", "NO"):
        return RULE_SKIP

    is_cluster  = cluster_val  == "YES"
    is_liveplay = liveplay_val == "YES"
...
def compute_bc_results(servers_qs, datacenter: str, rule_texts: dict) -> list[BCResult]:
    ...
    for server in servers_qs.select_related('server_unique'):
        ...
        # Skip servers with undetermined cluster or in_live_play values
        if rule_key is RULE_SKIP:
            continue
