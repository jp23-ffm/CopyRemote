document.addEventListener('DOMContentLoaded', function () {
    ...
    const dateFieldInputnames = new Set();      // e.g. {'obsopurchasedate', 'obso', ...}
    const dateInputnameToField = {};            // 'obsopurchasedate' -> 'OBSO_HWPURCHASEDATE'
    ...
    function clearFilters() {
        ...
        localStorage.removeItem('selectedSearchName_' + _appname);
    ...
    function createFilterTag(column, term) {
        ...
        removeButton.addEventListener('click', () => {
            ...
            // If no tags remain, clear the saved search label before reload
            if (filterTagsDiv.children.length === 0) {
                localStorage.removeItem('selectedSearchName_' + ((typeof appname !== 'undefined') ? appname : 'app'));
            }
    ...
    function updateDateRangeTag(baseInputname) {
        ...
        // Remove existing tag for this date field
    ...
    // Init Flatpickr on date inputs and attach change handlers
        ...
        document.querySelectorAll('.date-from, .date-to').forEach(input => {
            flatpickr(input, {
                ...
                onReady: function(selectedDates, dateStr, instance) {
                    // Restore date from URL on init (Flatpickr replaces the input value)
	...
	if (saveSearchForm) {
		...
		saveSearchForm.addEventListener('submit', function(event) {
        	...
        	// Include column order if any
        	const columnOrder = sessionStorage.getItem('columnOrder');
        	if (columnOrder) {
        	    filters['column_order'] = columnOrder;
        	}
    ...
    for (const [key, value] of urlParams.entries()) {
        ...
        // Skip date range params — restored separately below
    ...
    // Rebuild date range tags from restored input values
    for (const baseInputname of dateFieldInputnames) {
        updateDateRangeTag(baseInputname);
    }
    ...
    function initColumnDragDrop() {
        ...
        const draggableHeaders = table.querySelectorAll('thead th[draggable="true"]');

        draggableHeaders.forEach(header => {
            header.addEventListener('dragstart', (e) => {
                if (window._columnResizing) { e.preventDefault(); return; }
                draggedTh = header;
            ...
            header.addEventListener('dragend', () => {
                ...
                table.querySelectorAll('thead th').forEach(h => h.classList.remove('drag-over-left', 'drag-over-right'));
                draggedTh = null;
            });
            ...
            header.addEventListener('dragover', (e) => {
                ...
                if (draggedTh && draggedTh !== header) {
                    const allHeaders = Array.from(table.querySelectorAll('thead th'));
                    const fromIndex = allHeaders.indexOf(draggedTh);
                    const toIndex = allHeaders.indexOf(header);
                    header.classList.remove('drag-over-left', 'drag-over-right');
                    header.classList.add(fromIndex < toIndex ? 'drag-over-right' : 'drag-over-left');
                }
            });

            header.addEventListener('dragleave', () => { header.classList.remove('drag-over-left', 'drag-over-right'); });

            header.addEventListener('drop', (e) => {
                e.preventDefault();
                header.classList.remove('drag-over-left', 'drag-over-right');
                if (!draggedTh || draggedTh === header) return;

                const allHeaders = Array.from(table.querySelectorAll('thead th'));
                const fromIndex = allHeaders.indexOf(draggedTh);
                const toIndex = allHeaders.indexOf(header);

                if (fromIndex < FIXED_COLUMNS || toIndex < FIXED_COLUMNS) return;

                moveColumn(table, fromIndex, toIndex);
                saveColumnOrder(table, FIXED_COLUMNS);
            });
        });
    }
    ...
    function moveColumn(table, fromIndex, toIndex) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const refCell = row.cells[toIndex];
    ...
    function saveColumnOrder(table, fixedCols) {
        const order = Array.from(table.querySelectorAll('thead th'))
            .slice(fixedCols)
            .map(th => th.getAttribute('data-column'))
            .filter(Boolean);
        sessionStorage.setItem('columnOrder', JSON.stringify(order));
    }
    ...
    function restoreColumnOrder() {
        ...
        // Priority: URL param (from SavedSearch load) > sessionStorage
        const urlParams = new URLSearchParams(window.location.search);
        let orderStr = urlParams.get('column_order');
        let fromUrl = false;

        if (orderStr) {
            fromUrl = true;
        } else {
            orderStr = sessionStorage.getItem('columnOrder');
        }
        if (!orderStr) return;

        let targetOrder;
        try {
            targetOrder = JSON.parse(orderStr);
        } catch {
            targetOrder = orderStr.split(',');
        }
        if (!Array.isArray(targetOrder) || targetOrder.length === 0) return;

        reorderAllColumns(table, FIXED_COLUMNS, targetOrder);

        // Persist to sessionStorage (if from URL, transfer there)
        sessionStorage.setItem('columnOrder', JSON.stringify(targetOrder));

        // Clean URL param
        if (fromUrl) {
            urlParams.delete('column_order');
            const qs = urlParams.toString();
            history.replaceState(null, '', window.location.pathname + (qs ? '?' + qs : ''));
        }
    }

    function reorderAllColumns(table, fixedCols, targetOrder) {
        const rows = table.rows;
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const allCells = Array.from(row.cells);
            const dataCells = allCells.slice(fixedCols);

            // Map column name -> cell (th uses data-column, td uses data-column or class)
            const cellMap = {};
            ...
            // Append cells in target order
            targetOrder.forEach(colName => {
                const cell = cellMap[colName];
                if (cell) row.appendChild(cell);
            });

            // Append any remaining cells not in targetOrder (new columns added since save)
            dataCells.forEach(cell => {
                const colName = cell.getAttribute('data-column');
                if (colName && !targetOrder.includes(colName)) {
    ...
    window.fixStickyColumns = fixStickyColumns;
